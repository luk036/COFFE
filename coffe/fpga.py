...
# This one is higher because we had register-feedback coming into this mux.
DELAY_WEIGHT_LUT_C = 0.0704
...
class _SwitchBlockMUX(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        self.level1_size = int(
            math.ceil(float(self.required_size)/self.level2_size))
        ...
        if not self.use_tgate:
            self.transistor_names, self.wire_names = mux_subcircuits.generate_ptran_2lvl_mux(
                subcircuit_filename, self.name, self.implemented_size, self.level1_size, self.level2_size)
            ...
            self.initial_transistor_sizes["ptran_" +
                                          self.name + "_L1_nmos"] = 3
            self.initial_transistor_sizes["ptran_" +
                                          self.name + "_L2_nmos"] = 4
            ...
            self.transistor_names, self.wire_names = mux_subcircuits.generate_tgate_2lvl_mux(
                subcircuit_filename, self.name, self.implemented_size, self.level1_size, self.level2_size)
            # Initialize transistor sizes (to something more reasonable than all min size, but not necessarily a good choice, depends on architecture params)
            self.initial_transistor_sizes["tgate_" +
                                          self.name + "_L1_nmos"] = 3
            self.initial_transistor_sizes["tgate_" +
                                          self.name + "_L1_pmos"] = 3
            self.initial_transistor_sizes["tgate_" +
                                          self.name + "_L2_nmos"] = 4
            self.initial_transistor_sizes["tgate_" +
                                          self.name + "_L2_pmos"] = 4
            self.initial_transistor_sizes["inv_" + self.name + "_1_nmos"] = 8
            self.initial_transistor_sizes["inv_" + self.name + "_1_pmos"] = 4
            self.initial_transistor_sizes["inv_" + self.name + "_2_nmos"] = 10
            self.initial_transistor_sizes["inv_" + self.name + "_2_pmos"] = 20
    ...
    def update_area(self, area_dict, width_dict):
        ...
        area_with_sram = (
            area + (self.level1_size + self.level2_size)*area_dict["sram"])
        ...
        # Update VPR areas
        if not self.use_tgate:
            area_dict["switch_mux_trans_size"] = area_dict["ptran_" +
                                                           self.name + "_L1"]
            area_dict["switch_buf_size"] = area_dict["rest_" + self.name + ""] + \
                area_dict["inv_" + self.name + "_1"] + \
                area_dict["inv_" + self.name + "_2"]
        else:
            area_dict["switch_mux_trans_size"] = area_dict["tgate_" +
                                                           self.name + "_L1"]
            area_dict["switch_buf_size"] = area_dict["inv_" +
                                                     self.name + "_1"] + area_dict["inv_" + self.name + "_2"]

    def update_wires(self, width_dict, wire_lengths, wire_layers, ratio):
        """ Update wire lengths and wire layers based on the width of things, obtained from width_dict. """

        # Update wire lengths
        wire_lengths["wire_" + self.name + "_driver"] = (
            width_dict["inv_" + self.name + "_1"] + width_dict["inv_" + self.name + "_2"])/4
        wire_lengths["wire_" + self.name +
                     "_L1"] = width_dict[self.name] * ratio
        wire_lengths["wire_" + self.name +
                     "_L2"] = width_dict[self.name] * ratio
    ...
    def print_details(self, report_file):
        ...
        utils.print_and_write(
            report_file, "  Required MUX size: " + str(self.required_size) + ":1")
        utils.print_and_write(
            report_file, "  Implemented MUX size: " + str(self.implemented_size) + ":1")
        utils.print_and_write(
            report_file, "  Level 1 size = " + str(self.level1_size))
        utils.print_and_write(
            report_file, "  Level 2 size = " + str(self.level2_size))
        utils.print_and_write(
            report_file, "  Number of unused inputs = " + str(self.num_unused_inputs))
        utils.print_and_write(
            report_file, "  Number of MUXes per tile: " + str(self.num_per_tile))
        utils.print_and_write(
            report_file, "  Number of SRAM cells per MUX: " + str(self.sram_per_mux))
...
class _ConnectionBlockMUX(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if not self.use_tgate:
            ...
            # Initialize transistor sizes (to something more reasonable than all min size, but not necessarily a good choice, depends on architecture params)
            self.initial_transistor_sizes["ptran_" +
                                          self.name + "_L1_nmos"] = 2
            self.initial_transistor_sizes["ptran_" +
                                          self.name + "_L2_nmos"] = 2
            self.initial_transistor_sizes["rest_" + self.name + "_pmos"] = 1
            ...
            self.initial_transistor_sizes["tgate_" +
                                          self.name + "_L1_nmos"] = 2
            self.initial_transistor_sizes["tgate_" +
                                          self.name + "_L1_pmos"] = 2
            self.initial_transistor_sizes["tgate_" +
                                          self.name + "_L2_nmos"] = 2
            self.initial_transistor_sizes["tgate_" +
                                          self.name + "_L2_pmos"] = 2
            self.initial_transistor_sizes["inv_" + self.name + "_1_nmos"] = 2
            self.initial_transistor_sizes["inv_" + self.name + "_1_pmos"] = 2
            self.initial_transistor_sizes["inv_" + self.name + "_2_nmos"] = 6
            self.initial_transistor_sizes["inv_" + self.name + "_2_pmos"] = 12
    ...
    def generate_top(self):
        ...
        self.top_spice_path = top_level.generate_connection_block_top(
            self.name)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        if not self.use_tgate:
            area_dict["ipin_mux_trans_size"] = area_dict["ptran_" +
                                                         self.name + "_L1"]
            area_dict["cb_buf_size"] = area_dict["rest_" + self.name + ""] + \
                area_dict["inv_" + self.name + "_1"] + \
                area_dict["inv_" + self.name + "_2"]
        else:
            area_dict["ipin_mux_trans_size"] = area_dict["tgate_" +
                                                         self.name + "_L1"]
            area_dict["cb_buf_size"] = area_dict["inv_" +
                                                 self.name + "_1"] + area_dict["inv_" + self.name + "_2"]
...
class _LocalMUX(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if not self.use_tgate:
            # Call MUX generation function
            self.transistor_names, self.wire_names = mux_subcircuits.generate_ptran_2lvl_mux_no_driver(
                subcircuit_filename, self.name, self.implemented_size, self.level1_size, self.level2_size)
            ...
            # Call MUX generation function
            self.transistor_names, self.wire_names = mux_subcircuits.generate_tgate_2lvl_mux_no_driver(
                subcircuit_filename, self.name, self.implemented_size, self.level1_size, self.level2_size)
...
class _LUTInputDriver(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if not self.use_tgate:
            self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut_driver(
                subcircuit_filename, self.name, self.type)
        else:
            self.transistor_names, self.wire_names = lut_subcircuits.generate_tgate_lut_driver(
                subcircuit_filename, self.name, self.type)

        # Initialize transistor sizes (to something more reasonable than all min size, but not necessarily a good choice, depends on architecture params)
        if not self.use_tgate:
            if self.type != "default":
                self.initial_transistor_sizes["inv_" +
                                              self.name + "_0_nmos"] = 2
                self.initial_transistor_sizes["inv_" +
                                              self.name + "_0_pmos"] = 2
            if self.type == "reg_fb" or self.type == "reg_fb_rsel":
                self.initial_transistor_sizes["ptran_" +
                                              self.name + "_0_nmos"] = 2
                self.initial_transistor_sizes["rest_" +
                                              self.name + "_pmos"] = 1
            if self.type != "default":
                self.initial_transistor_sizes["inv_" +
                                              self.name + "_1_nmos"] = 1
    ...
    def generate_top(self):
        ...
        self.top_spice_path = top_level.generate_lut_driver_top(
            self.name, self.type)
        # And, generate the LUT driver + LUT path top level file. We use this file to measure total delay through the LUT.
        top_level.generate_lut_and_driver_top(
            self.name, self.type, self.use_tgate, self.use_fluts)
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers):
        ...
        if not self.use_tgate:
            ...
            if self.type == "default_rsel" or self.type == "reg_fb_rsel":
                wire_lengths["wire_" + self.name + "_0_rsel"] = width_dict[self.name] / \
                    4 + width_dict["lut"] + width_dict["ff"]/4
                wire_layers["wire_" + self.name + "_0_rsel"] = 0
            if self.type == "default_rsel":
                wire_lengths["wire_" + self.name + "_0_out"] = width_dict["inv_" +
                                                                          self.name + "_0"]/4 + width_dict["inv_" + self.name + "_2"]/4
                wire_layers["wire_" + self.name + "_0_out"] = 0
            if self.type == "reg_fb" or self.type == "reg_fb_rsel":
                wire_lengths["wire_" + self.name + "_0_out"] = width_dict["inv_" +
                                                                          self.name + "_0"]/4 + width_dict["ptran_" + self.name + "_0"]/4
                wire_layers["wire_" + self.name + "_0_out"] = 0
                wire_lengths["wire_" + self.name +
                             "_0"] = width_dict["ptran_" + self.name + "_0"]
                wire_layers["wire_" + self.name + "_0"] = 0
            if self.type == "default":
                wire_lengths["wire_" + self.name] = width_dict["local_mux"] / \
                    4 + width_dict["inv_" + self.name + "_2"]/4
                wire_layers["wire_" + self.name] = 0
            else:
                wire_lengths["wire_" + self.name] = width_dict["inv_" +
                                                               self.name + "_1"]/4 + width_dict["inv_" + self.name + "_2"]/4
                wire_layers["wire_" + self.name] = 0
            ...
            if self.type == "reg_fb" or self.type == "reg_fb_rsel":
                wire_lengths["wire_" + self.name + "_0_out"] = width_dict["inv_" +
                                                                          self.name + "_0"]/4 + width_dict["tgate_" + self.name + "_0"]/4
                wire_layers["wire_" + self.name + "_0_out"] = 0
                wire_lengths["wire_" + self.name +
                             "_0"] = width_dict["tgate_" + self.name + "_0"]
                wire_layers["wire_" + self.name + "_0"] = 0
            if self.type == "default":
                wire_lengths["wire_" + self.name] = width_dict["local_mux"] / \
                    4 + width_dict["inv_" + self.name + "_2"]/4
...
class _LUTInputNotDriver(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if not self.use_tgate:
            self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut_not_driver(
                subcircuit_filename, self.name)
        else:
            self.transistor_names, self.wire_names = lut_subcircuits.generate_tgate_lut_not_driver(
                subcircuit_filename, self.name)
    ...
    def generate_top(self):
        ...
        self.top_spice_path = top_level.generate_lut_driver_not_top(
            self.name, self.type)
...
class _LUTInput(_CompoundCircuit):
    ...
    def __init__(self, name, Rsel, Rfb, delay_weight, use_tgate, use_fluts):
        ...
        self.driver = _LUTInputDriver(
            name, self.type, delay_weight, use_tgate, use_fluts)
        # Create LUT input not driver
        self.not_driver = _LUTInputNotDriver(
            name, self.type, delay_weight, use_tgate)
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        print("Generating lut " + self.name +
              "-input driver (" + self.type + ")")

        # Generate the driver
        init_tran_sizes = self.driver.generate(
            subcircuit_filename, min_tran_width)
        # Generate the not driver
        init_tran_sizes.update(self.not_driver.generate(
            subcircuit_filename, min_tran_width))
    ...
    def print_details(self, report_file):
        ...
        utils.print_and_write(report_file, "  LUT input " +
...
class _LUTInputDriverLoad:
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers, ratio):
        ...
        # Update wire lengths
        wire_lengths["wire_lut_" + self.name +
                     "_driver_load"] = width_dict["lut"] * ratio
    ...
    def generate(self, subcircuit_filename, K):
        ...
        if not self.use_tgate:
            ...
            if self.name == "a":
                self.wire_names = lut_subcircuits.generate_ptran_lut_driver_load(
                    subcircuit_filename, self.name, K, self.use_fluts)
            elif self.name == "b":
                self.wire_names = lut_subcircuits.generate_ptran_lut_driver_load(
                    subcircuit_filename, self.name, K, self.use_fluts)
            elif self.name == "c":
            ...
            # Call generation function based on input
            if self.name == "a":
                self.wire_names = lut_subcircuits.generate_tgate_lut_driver_load(
                    subcircuit_filename, self.name, K, self.use_fluts)
            elif self.name == "b":
                self.wire_names = lut_subcircuits.generate_tgate_lut_driver_load(
                    subcircuit_filename, self.name, K, self.use_fluts)
...
class _LUT(_SizableCircuit):
    ...
    def __init__(self, K, Rsel, Rfb, use_tgate, use_finfet, use_fluts):
        ...
        # Delay weight in a representative critical path
        self.delay_weight = DELAY_WEIGHT_LUT_A + DELAY_WEIGHT_LUT_B + \
            DELAY_WEIGHT_LUT_C + DELAY_WEIGHT_LUT_D
        if K >= 5:
        ...
        for i in range(tempK):
            ...
            if name == "a":
                ...
                raise Exception(
                    "No delay weight definition for LUT input " + name)
            self.input_drivers[name] = _LUTInput(
                name, Rsel, Rfb, delay_weight, use_tgate, use_fluts)
            self.input_driver_loads[name] = _LUTInputDriverLoad(
                name, use_tgate, use_fluts)
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if tempK == 6:
            init_tran_sizes = self._generate_6lut(
                subcircuit_filename, min_tran_width, self.use_tgate, self.use_finfet, self.use_fluts)
        elif tempK == 5:
            init_tran_sizes = self._generate_5lut(
                subcircuit_filename, min_tran_width, self.use_tgate, self.use_finfet, self.use_fluts)
        elif tempK == 4:
            init_tran_sizes = self._generate_4lut(
                subcircuit_filename, min_tran_width, self.use_tgate, self.use_finfet, self.use_fluts)
    ...
    def generate_top(self):
        ...
        if tempK == 6:
            self.top_spice_path = top_level.generate_lut6_top(
                self.name, self.use_tgate)
        elif tempK == 5:
            self.top_spice_path = top_level.generate_lut5_top(
                self.name, self.use_tgate)
        elif tempK == 4:
            self.top_spice_path = top_level.generate_lut4_top(
                self.name, self.use_tgate)
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers, lut_ratio):
        ...
        if not self.use_tgate:
            if self.K == 6:
                # Update wire lengths
                wire_lengths["wire_lut_sram_driver"] = (
                    width_dict["inv_lut_0sram_driver_2"] + width_dict["inv_lut_0sram_driver_2"])/4
                wire_lengths["wire_lut_sram_driver_out"] = (
                    width_dict["inv_lut_0sram_driver_2"] + width_dict["ptran_lut_L1"])/4
                ...
                wire_lengths["wire_lut_int_buffer"] = (
                    width_dict["inv_lut_int_buffer_1"] + width_dict["inv_lut_int_buffer_2"])/4
                wire_lengths["wire_lut_int_buffer_out"] = (
                    width_dict["inv_lut_int_buffer_2"] + width_dict["ptran_lut_L4"])/4
                ...
                wire_lengths["wire_lut_out_buffer"] = (
                    width_dict["inv_lut_out_buffer_1"] + width_dict["inv_lut_out_buffer_2"])/4
            ...
            if self.K == 6:
                ...
                wire_lengths["wire_lut_sram_driver"] = (
                    width_dict["inv_lut_0sram_driver_2"] + width_dict["inv_lut_0sram_driver_2"])/4
                wire_lengths["wire_lut_sram_driver_out"] = (
                    width_dict["inv_lut_0sram_driver_2"] + width_dict["tgate_lut_L1"])/4
                ...
                wire_lengths["wire_lut_int_buffer"] = (
                    width_dict["inv_lut_int_buffer_1"] + width_dict["inv_lut_int_buffer_2"])/4
                wire_lengths["wire_lut_int_buffer_out"] = (
                    width_dict["inv_lut_int_buffer_2"] + width_dict["tgate_lut_L4"])/4
        ...
        for driver_load_name, input_driver_load in self.input_driver_loads.items():
            input_driver_load.update_wires(
                width_dict, wire_lengths, wire_layers, lut_ratio)
    ...
    def print_details(self, report_file):
        ...
        utils.print_and_write(
            report_file, "  Internal buffering: 2-stage buffer betweens levels 3 and 4")
        utils.print_and_write(
            report_file, "  Isolation inverters between SRAM and LUT inputs")
        utils.print_and_write(report_file, "")
    ...
    def _generate_6lut(self, subcircuit_filename, min_tran_width, use_tgate, use_finfet, use_fluts):
        ...
        if not use_tgate:
            ...
            self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut6(
                subcircuit_filename, min_tran_width, use_finfet)
            ...
            self.transistor_names, self.wire_names = lut_subcircuits.generate_tgate_lut6(
                subcircuit_filename, min_tran_width, use_finfet)
    ...
    def _generate_5lut(self, subcircuit_filename, min_tran_width, use_tgate, use_finfet, use_fluts):
        ...
        if not use_tgate:
            # use pass transistor
            self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut5(
                subcircuit_filename, min_tran_width, use_finfet)
            # Give initial transistor sizes
            self.initial_transistor_sizes["inv_lut_0sram_driver_2_nmos"] = 4
            self.initial_transistor_sizes["inv_lut_0sram_driver_2_pmos"] = 6
            ...
            # use transmission gates
            self.transistor_names, self.wire_names = lut_subcircuits.generate_tgate_lut5(
                subcircuit_filename, min_tran_width, use_finfet)
            # Give initial transistor sizes
            self.initial_transistor_sizes["inv_lut_0sram_driver_2_nmos"] = 4
            self.initial_transistor_sizes["inv_lut_0sram_driver_2_pmos"] = 6
    ...
    def _generate_4lut(self, subcircuit_filename, min_tran_width, use_tgate, use_finfet, use_fluts):
        ...
        if not use_tgate:
            # use pass transistor
            self.transistor_names, self.wire_names = lut_subcircuits.generate_ptran_lut4(
                subcircuit_filename, min_tran_width, use_finfet)
            # Give initial transistor sizes
            self.initial_transistor_sizes["inv_lut_0sram_driver_2_nmos"] = 4
            self.initial_transistor_sizes["inv_lut_0sram_driver_2_pmos"] = 6
            ...
            # use transmission gates
            self.transistor_names, self.wire_names = lut_subcircuits.generate_tgate_lut4(
                subcircuit_filename, min_tran_width, use_finfet)
            # Give initial transistor sizes
            self.initial_transistor_sizes["inv_lut_0sram_driver_2_nmos"] = 4
            self.initial_transistor_sizes["inv_lut_0sram_driver_2_pmos"] = 6
...
class _CarryChainMux(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width, use_finfet):
        ...
        if not self.use_tgate:
            self.transistor_names, self.wire_names = mux_subcircuits.generate_ptran_2_to_1_mux(
                subcircuit_filename, self.name)
            # Initialize transistor sizes (to something more reasonable than all min size, but not necessarily a good choice, depends on architecture params)
            ...
            self.transistor_names, self.wire_names = mux_subcircuits.generate_tgate_2_to_1_mux(
                subcircuit_filename, self.name)
            # Initialize transistor sizes (to something more reasonable than all min size, but not necessarily a good choice, depends on architecture params)
    ...
    def generate_top(self):
        ...
        self.top_spice_path = top_level.generate_cc_mux_top(
            self.name, self.use_tgate)
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers):
        ...
        if not self.use_tgate:
            wire_lengths["wire_" +
                         self.name] = width_dict["ptran_" + self.name]
        else:
            wire_lengths["wire_" +
                         self.name] = width_dict["tgate_" + self.name]

        wire_lengths["wire_" + self.name + "_driver"] = (
            width_dict["inv_" + self.name + "_1"] + width_dict["inv_" + self.name + "_1"])/4
...
class _CarryChainPer(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width, use_finfet):
        ...
        self.transistor_names, self.wire_names = lut_subcircuits.generate_carry_chain_perf_ripple(
            subcircuit_filename, self.name, use_finfet)
        self.initial_transistor_sizes["inv_" + self.name + "_1_nmos"] = 1
        self.initial_transistor_sizes["inv_" + self.name + "_1_pmos"] = 1
    ...
    def generate_top(self):
        ...
        if self.carry_chain_type == "ripple":
            self.top_spice_path = top_level.generate_carry_chain_ripple_top(
                self.name)
        else:
            self.top_spice_path = top_level.generate_carry_chain_skip_top(
                self.name, self.use_tgate)
...
class _CarryChain(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width, use_finfet):
        ...
        self.transistor_names, self.wire_names = lut_subcircuits.generate_full_adder_simplified(
            subcircuit_filename, self.name, use_finfet)
    ...
    def update_area(self, area_dict, width_dict):
        """ Calculate Carry Chain area and update dictionaries. """
        area = area_dict["inv_carry_chain_1"] * 2 + area_dict["inv_carry_chain_2"] + \
            area_dict["tgate_carry_chain_1"] * 4 + \
            area_dict["tgate_carry_chain_2"] * 4
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers):
        ...
        if self.FAs_per_flut == 2:
            # Wire for input A
            wire_lengths["wire_" + self.name +
                         "_1"] = width_dict["lut_and_drivers"]
        else:
            wire_lengths["wire_" + self.name +
                         "_1"] = width_dict[self.name]  # Wire for input A
        wire_layers["wire_" + self.name + "_1"] = 0
        wire_lengths["wire_" + self.name +
                     "_2"] = width_dict[self.name]  # Wire for input B
        wire_layers["wire_" + self.name + "_2"] = 0
        if self.FAs_per_flut == 1:
            # Wire for input Cin
            wire_lengths["wire_" + self.name +
                         "_3"] = width_dict["logic_cluster"]/(2 * self.N)
        else:
            # Wire for input Cin
            wire_lengths["wire_" + self.name +
                         "_3"] = width_dict["logic_cluster"]/(4 * self.N)
        wire_layers["wire_" + self.name + "_3"] = 0
        if self.FAs_per_flut == 1:
            # Wire for output Cout
            wire_lengths["wire_" + self.name +
                         "_4"] = width_dict["logic_cluster"]/(2 * self.N)
        else:
            # Wire for output Cout
            wire_lengths["wire_" + self.name +
                         "_4"] = width_dict["logic_cluster"]/(4 * self.N)
        wire_layers["wire_" + self.name + "_4"] = 0
        wire_lengths["wire_" + self.name +
                     "_5"] = width_dict[self.name]  # Wire for output Sum
...
class _CarryChainSkipAnd(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width, use_finfet):
        ...
        self.transistor_names, self.wire_names = lut_subcircuits.generate_skip_and_tree(
            subcircuit_filename, self.name, use_finfet, self.nand1_size, self.nand2_size)

        self.initial_transistor_sizes["inv_nand" +
                                      str(self.nand1_size)+"_xcarry_chain_and_1_nmos"] = 1
        self.initial_transistor_sizes["inv_nand" +
                                      str(self.nand1_size)+"_xcarry_chain_and_1_pmos"] = 1
        self.initial_transistor_sizes["inv_xcarry_chain_and_2_nmos"] = 1
        self.initial_transistor_sizes["inv_xcarry_chain_and_2_pmos"] = 1
        self.initial_transistor_sizes["inv_nand" +
                                      str(self.nand2_size)+"_xcarry_chain_and_3_nmos"] = 1
        self.initial_transistor_sizes["inv_nand" +
                                      str(self.nand2_size)+"_xcarry_chain_and_3_pmos"] = 1
    ...
    def generate_top(self):
        ...
        self.top_spice_path = top_level.generate_carrychainand_top(
            self.name, self.use_tgate, self.nand1_size, self.nand2_size)

    def update_area(self, area_dict, width_dict):
        """ Calculate Carry Chain area and update dictionaries. """
        area_1 = (area_dict["inv_nand"+str(self.nand1_size)+"_xcarry_chain_and_1"] +
                  area_dict["inv_xcarry_chain_and_2"]) * int(math.ceil(float(self.skip_size/self.nand1_size)))
        area_2 = area_dict["inv_nand"+str(self.nand2_size) +
                           "_xcarry_chain_and_3"] + area_dict["inv_xcarry_chain_and_4"]
        area = area_1 + area_2
        area_with_sram = area
        width = math.sqrt(area)
        area_dict[self.name] = area
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers):
        ...
        if self.FAs_per_flut == 2:
            wire_lengths["wire_" + self.name +
                         "_1"] = (width_dict["ble"]*self.skip_size)/4.0
        else:
            wire_lengths["wire_" + self.name +
                         "_1"] = (width_dict["ble"]*self.skip_size)/2.0
        wire_layers["wire_" + self.name + "_1"] = 0
...
class _CarryChainInterCluster(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width, use_finfet):
        ...
        self.transistor_names, self.wire_names = lut_subcircuits.generate_carry_inter(
            subcircuit_filename, self.name, use_finfet)
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers):
        ...
        wire_lengths["wire_" + self.name +
                     "_1"] = width_dict["tile"] * self.inter_wire_length
        wire_layers["wire_" + self.name + "_1"] = 0
...
class _CarryChainSkipMux(_SizableCircuit):
    ...
    def generate_top(self):
        ...
        print("Generating top-level " + self.name)
        self.top_spice_path = top_level.generate_skip_mux_top(
            self.name, self.use_tgate)
...
class _FlipFlop:
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if self.register_select == 'z':
            print("Generating FF")
            if not self.use_tgate:
                self.transistor_names, self.wire_names = ff_subcircuits.generate_ptran_d_ff(
                    subcircuit_filename, self.use_finfet)
            else:
                self.transistor_names, self.wire_names = ff_subcircuits.generate_tgate_d_ff(
                    subcircuit_filename, self.use_finfet)

        else:
            print("Generating FF with register select on BLE input " +
                  self.register_select)
            if not self.use_tgate:
                self.transistor_names, self.wire_names = ff_subcircuits.generate_ptran_2_input_select_d_ff(
                    subcircuit_filename, self.use_finfet)
            else:
                self.transistor_names, self.wire_names = ff_subcircuits.generate_tgate_2_input_select_d_ff(
                    subcircuit_filename, self.use_finfet)
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers):
        ...
        wire_lengths["wire_ff_input_out"] = (
            width_dict["inv_ff_input_1"] + width_dict["tgate_ff_1"])/4
        wire_lengths["wire_ff_tgate_1_out"] = (
            width_dict["tgate_ff_1"] + width_dict["inv_ff_cc1_1"])/4
        wire_lengths["wire_ff_cc1_out"] = (
            width_dict["inv_ff_cc1_1"] + width_dict["tgate_ff_2"])/4
        wire_lengths["wire_ff_tgate_2_out"] = (
            width_dict["tgate_ff_2"] + width_dict["inv_ff_cc1_2"])/4
        wire_lengths["wire_ff_cc2_out"] = (
            width_dict["inv_ff_cc1_2"] + width_dict["inv_ff_output_driver"])/4
...
class _LocalBLEOutput(_SizableCircuit):
    ...
    def generate_top(self):
        print("Generating top-level " + self.name)
        self.top_spice_path = top_level.generate_local_ble_output_top(
            self.name, self.use_tgate)
...
class _GeneralBLEOutput(_SizableCircuit):
    ...
    def generate_top(self):
        print("Generating top-level " + self.name)
        self.top_spice_path = top_level.generate_general_ble_output_top(
            self.name, self.use_tgate)
...
class _LUTOutputLoad:
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        self.wire_names = load_subcircuits.generate_lut_output_load(
            subcircuit_filename, self.num_local_outputs, self.num_general_outputs)

    def update_wires(self, width_dict, wire_lengths, wire_layers):
        """ Update wire lengths and wire layers based on the width of things, obtained from width_dict. """

        # Update wire lengths
        wire_lengths["wire_lut_output_load_1"] = (
            width_dict["ff"] + width_dict["lut_and_drivers"])/8
...
class _flut_mux(_CompoundCircuit):
    ...
    def generate_top(self):
        ...
        print("Generating top-level " + self.name)
        self.top_spice_path = top_level.generate_flut_mux_top(
            self.name, self.use_tgate, self.enable_carry_chain)
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers, lut_ratio):
        ...
        if not self.use_tgate:
            wire_lengths["wire_" +
                         self.name] = width_dict["ptran_" + self.name]
            wire_lengths["wire_lut_to_flut_mux"] = width_dict["lut"] / \
                2 * lut_ratio
        else:
            wire_lengths["wire_" +
                         self.name] = width_dict["tgate_" + self.name]
...
class _BLE(_CompoundCircuit):
    ...
    def __init__(self, K, Or, Ofb, Rsel, Rfb, use_tgate, use_finfet, use_fluts, enable_carry_chain, FAs_per_flut, carry_skip_periphery_count, N):
        ...
        self.lut_output_load = _LUTOutputLoad(
            self.num_local_outputs, self.num_general_outputs)
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        # Generate LUT and FF
        init_tran_sizes = {}
        init_tran_sizes.update(self.lut.generate(
            subcircuit_filename, min_tran_width))
        init_tran_sizes.update(self.ff.generate(
            subcircuit_filename, min_tran_width))
        ...
        init_tran_sizes.update(self.general_output.generate(subcircuit_filename,
                                                            min_tran_width))
        load_subcircuits.generate_ble_outputs(
            subcircuit_filename, self.num_local_outputs, self.num_general_outputs)

        # flut mux
        if self.use_fluts:
            init_tran_sizes.update(self.fmux.generate(
                subcircuit_filename, min_tran_width))
        # Generate LUT load
    ...
    def update_area(self, area_dict, width_dict):
        ...
        local_ble_output_area = self.num_local_outputs * \
            self.local_output.update_area(area_dict, width_dict)
        general_ble_output_area = self.num_general_outputs * \
            self.general_output.update_area(area_dict, width_dict)
        ...
        if self.enable_carry_chain == 1:
            if self.carry_skip_periphery_count == 0:
                ble_area = ble_area + area_dict["carry_chain"] * self.FAs_per_flut + (
                    self.FAs_per_flut) * area_dict["carry_chain_mux"]
            else:
                ble_area = ble_area + area_dict["carry_chain"] * self.FAs_per_flut + (
                    self.FAs_per_flut) * area_dict["carry_chain_mux"]
                ble_area = ble_area + \
                    ((area_dict["xcarry_chain_and"] + area_dict["xcarry_chain_mux"])
                     * self.carry_skip_periphery_count)/self.N
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers, lut_ratio):
        ...
        wire_lengths["wire_ble_outputs"] = self.num_local_outputs * \
            width_dict["local_ble_output"] + \
            self.num_general_outputs*width_dict["general_ble_output"]
        wire_layers["wire_ble_outputs"] = 0

        # Update LUT load wires
        self.lut_output_load.update_wires(
            width_dict, wire_lengths, wire_layers)

        # Fracturable luts:
        if self.use_fluts:
            self.fmux.update_wires(
                width_dict, wire_lengths, wire_layers, lut_ratio)
...
class _GeneralBLEOutputLoad:
    ...
    def generate(self, subcircuit_filename, specs, sb_mux):
        ...
        self._compute_load(
            specs, sb_mux, self.channel_usage_assumption, self.num_sb_mux_on_assumption)
        self.wire_names = load_subcircuits.generate_general_ble_output_load(
            subcircuit_filename, self.num_sb_mux_off, self.num_sb_mux_partial, self.num_sb_mux_on_assumption)
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers, h_dist, height):
        ...
        wire_lengths["wire_general_ble_output"] = width_dict["tile"] * \
            OUTPUT_TRACK_ACCESS_SPAN
        if height != 0.0:
    ...
    def print_details(self, report_file):
        ...
        utils.print_and_write(report_file, "  Total number of SB inputs connected to cluster output: " +
                              str(self.num_sb_mux_off + self.num_sb_mux_partial + self.num_sb_mux_on_assumption))
        utils.print_and_write(
            report_file, "  Number of 'on' SB MUXes (assumed): " + str(self.num_sb_mux_on_assumption))
        utils.print_and_write(
            report_file, "  Number of 'partial' SB MUXes: " + str(self.num_sb_mux_partial))
        utils.print_and_write(
            report_file, "  Number of 'off' SB MUXes: " + str(self.num_sb_mux_off))
        utils.print_and_write(report_file, "")
    ...
    def _compute_load(self, specs, sb_mux, channel_usage, sb_mux_on):
        ...
        sb_inputs_from_routing = sb_mux.required_size * \
            sb_mux.num_per_tile - sb_inputs_from_cluster_outputs
        frac_partial_paths_on_cluster_out = float(
            sb_inputs_from_cluster_outputs)/(sb_inputs_from_cluster_outputs+sb_inputs_from_routing)
        # The total number of partial paths on the cluster outputs is calculated using that fraction
        total_cluster_output_partial_paths = int(
            frac_partial_paths_on_cluster_out*total_partial_paths)
        # And we divide by the number of cluster outputs to get partial paths per output
        cluster_output_partial_paths = int(
            math.ceil(float(total_cluster_output_partial_paths)/specs.num_cluster_outputs))
...
class _LocalRoutingWireLoad:
    ...
    def generate(self, subcircuit_filename, specs, local_mux):
        ...
        # Generate SPICE deck
        self.wire_names = load_subcircuits.local_routing_load_generate(
            subcircuit_filename, self.on_inputs_per_wire, self.partial_inputs_per_wire, self.off_inputs_per_wire)
    ...
    def _compute_load(self, specs, local_mux):
        ...
        self.mux_inputs_per_wire = local_mux.implemented_size * \
            specs.N*specs.K/num_local_routing_wires
        ...
        self.on_inputs_per_wire = int(
            num_local_muxes_used/num_local_routing_wires)
        ...
        self.partial_inputs_per_wire = int(
            (local_mux.level2_size - 1.0)*num_local_muxes_used/num_local_routing_wires)
        # Make it at least 1
        if self.partial_inputs_per_wire < 1:
            self.partial_inputs_per_wire = 1

        # Number of off inputs is simply the difference
        self.off_inputs_per_wire = self.mux_inputs_per_wire - \
...
class _LogicCluster(_CompoundCircuit):
    ...
    def __init__(self, N, K, Or, Ofb, Rsel, Rfb, local_mux_size_required, num_local_mux_per_tile, use_tgate, use_finfet, use_fluts, enable_carry_chain, FAs_per_flut, carry_skip_periphery_count):
        ...
        self.ble = _BLE(K, Or, Ofb, Rsel, Rfb, use_tgate, use_finfet, use_fluts,
                        enable_carry_chain, FAs_per_flut, carry_skip_periphery_count, N)
        # Create local mux object
        self.local_mux = _LocalMUX(
            local_mux_size_required, num_local_mux_per_tile, use_tgate)
    ...
    def generate(self, subcircuits_filename, min_tran_width, specs):
        ...
        init_tran_sizes = {}
        init_tran_sizes.update(self.ble.generate(
            subcircuits_filename, min_tran_width))
        init_tran_sizes.update(self.local_mux.generate(
            subcircuits_filename, min_tran_width))
        self.local_routing_wire_load.generate(
            subcircuits_filename, specs, self.local_mux)
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers, ic_ratio, lut_ratio, ble_ic_dis, local_routing_wire_load_length):
        ...
        self.local_mux.update_wires(
            width_dict, wire_lengths, wire_layers, ic_ratio)
        self.local_routing_wire_load.update_wires(
            width_dict, wire_lengths, wire_layers, local_routing_wire_load_length)
        self.local_ble_output_load.update_wires(
            width_dict, wire_lengths, wire_layers, ble_ic_dis)
...
class _RoutingWireLoad:
    ...
    def generate(self, subcircuit_filename, specs, sb_mux, cb_mux):
        ...
        self._compute_load(specs, sb_mux, cb_mux, self.channel_usage_assumption,
                           self.cluster_input_usage_assumption)
        # Generate SPICE deck
        self.wire_names = load_subcircuits.general_routing_load_generate(
            subcircuit_filename, self.wire_length, self.tile_sb_on, self.tile_sb_partial, self.tile_sb_off, self.tile_cb_on, self.tile_cb_partial, self.tile_cb_off)
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers, height, num_sb_stripes, num_cb_stripes):
        ...
        if height != 0.0:
            if height > ((width_dict["tile"]*width_dict["tile"])/height):
                ...
                wire_lengths["wire_gen_routing"] = self.wire_length * \
                    ((width_dict["tile"]*width_dict["tile"])/height)
        ...
        if height != 0.0:
            if num_sb_stripes == 1:
                wire_lengths["wire_sb_load_on"] = wire_lengths["wire_gen_routing"] / \
                    self.wire_length
                wire_lengths["wire_sb_load_partial"] = wire_lengths["wire_gen_routing"] / \
                    self.wire_length
                wire_lengths["wire_sb_load_off"] = wire_lengths["wire_gen_routing"] / \
                    self.wire_length
            else:
                wire_lengths["wire_sb_load_on"] = wire_lengths["wire_gen_routing"] / \
                    (2*self.wire_length)
        ...
        wire_lengths["wire_cb_load_on"] = width_dict["tile"] * \
            INPUT_TRACK_ACCESS_SPAN
        wire_lengths["wire_cb_load_partial"] = width_dict["tile"] * \
            INPUT_TRACK_ACCESS_SPAN
        wire_lengths["wire_cb_load_off"] = width_dict["tile"] * \
            INPUT_TRACK_ACCESS_SPAN
        if height != 0 and num_cb_stripes == 1:
            wire_lengths["wire_cb_load_on"] = (
                wire_lengths["wire_gen_routing"]/self.wire_length) * 2
            wire_lengths["wire_cb_load_partial"] = (
                wire_lengths["wire_gen_routing"]/self.wire_length) * 2
            wire_lengths["wire_cb_load_off"] = (
                wire_lengths["wire_gen_routing"]/self.wire_length) * 2
        elif height != 0:
            wire_lengths["wire_cb_load_on"] = (
                wire_lengths["wire_gen_routing"]/self.wire_length)
    ...
    def print_details(self, report_file):
        ...
        utils.print_and_write(report_file, "  Number of SB inputs connected to routing wire = " +
                              str(self.sb_load_on + self.sb_load_partial + self.sb_load_off))
        utils.print_and_write(report_file, "  Wire: SB (on = " + str(self.sb_load_on) +
                              ", partial = " + str(self.sb_load_partial) + ", off = " + str(self.sb_load_off) + ")")
        utils.print_and_write(report_file, "  Number of CB inputs connected to routing wire = " +
                              str(self.cb_load_on + self.cb_load_partial + self.cb_load_off))
        utils.print_and_write(report_file, "  Wire: CB (on = " + str(self.cb_load_on) +
                              ", partial = " + str(self.cb_load_partial) + ", off = " + str(self.cb_load_off) + ")")
    ...
    def _compute_load(self, specs, sb_mux, cb_mux, channel_usage, cluster_input_usage):
        ...
        self.sb_load_partial = int(
            round(float(sb_level2_size - 1.0)*channel_usage))
        ...
        cb_load_on_probability = float(
            (I/2.0*cluster_input_usage*L))/(W*channel_usage)
        ...
        cb_load_partial_probability = (
            I/2*cluster_input_usage * (cb_level2_size - 1) * L) / W
        ...
        tile_sb_total_budget = tile_sb_on_budget + \
            tile_sb_partial_budget + tile_sb_off_budget
        ...
        tile_cb_total_budget = tile_cb_on_budget + \
            tile_cb_partial_budget + tile_cb_off_budget
...
class _pgateoutputcrossbar(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        # Call MUX generation function
        self.transistor_names, self.wire_names = memory_subcircuits.generate_pgateoutputcrossbar(
            subcircuit_filename, self.name, self.maxwidth, self.def_use_tgate)
    ...
    def generate_top(self):
        ...
        self.top_spice_path = top_level.generate_pgateoutputcrossbar_top(
            self.name, self.maxwidth, self.def_use_tgate)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        area = (area_dict["inv_" + self.name + "_1"] + area_dict["inv_" + self.name + "_2"] + area_dict["inv_" +
                self.name + "_3"] * 2) * self.maxwidth + area_dict["ptran_" + self.name + "_4"] * ptran_count
...
class _configurabledecoderiii(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if use_lp_transistor == 0:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_configurabledecoderiii(
                subcircuit_filename, self.name, self.required_size)
        else:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_configurabledecoderiii_lp(
                subcircuit_filename, self.name, self.required_size)

            print(self.transistor_names)
        # Initialize transistor sizes (to something more reasonable than all min size, but not necessarily a good choice, depends on architecture params)
        self.initial_transistor_sizes["inv_nand" +
                                      str(self.required_size)+"_" + self.name + "_1_nmos"] = 1
        self.initial_transistor_sizes["inv_nand" +
                                      str(self.required_size)+"_" + self.name + "_1_pmos"] = 1
        self.initial_transistor_sizes["inv_" + self.name + "_2_nmos"] = 5
        self.initial_transistor_sizes["inv_" + self.name + "_2_pmos"] = 5
    ...
    def generate_top(self):
        ...
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_configurabledecoderiii_top(
                self.name, self.fanin1, self.fanin2, self.required_size, self.tgatecount)
        else:
            self.top_spice_path = top_level.generate_configurabledecoderiii_top_lp(
                self.name, self.fanin1, self.fanin2, self.required_size, self.tgatecount)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        # predecoder area
        area = area_dict["inv_nand"+str(self.required_size)+"_" + self.name + "_1"] * \
            self.required_size + area_dict["inv_" + self.name + "_2"]
        area_with_sram = area
...
class _configurabledecoder3ii(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        # Call generation function
        if use_lp_transistor == 0:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_configurabledecoder3ii(
                subcircuit_filename, self.name)
        else:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_configurabledecoder3ii_lp(
                subcircuit_filename, self.name)
        # Initialize transistor sizes (to something more reasonable than all min size, but not necessarily a good choice, depends on architecture params)
    ...
    def generate_top(self):
        ...
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_configurabledecoder2ii_top(
                self.name, self.fan_out, 3)
        else:
            self.top_spice_path = top_level.generate_configurabledecoder2ii_top_lp(
                self.name, self.fan_out, 3)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        # predecoder area
        area = (area_dict["inv_nand3_" + self.name + "_1"] *
                3 + area_dict["inv_" + self.name + "_2"])*self.areafac
        area_with_sram = area
...
class _configurabledecoder2ii(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        # Call generation function
        if use_lp_transistor == 0:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_configurabledecoder2ii(
                subcircuit_filename, self.name)
        else:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_configurabledecoder2ii_lp(
                subcircuit_filename, self.name)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        # predecoder area
        area = (area_dict["inv_nand2_" + self.name + "_1"] *
                2 + area_dict["inv_" + self.name + "_2"])*self.areafac
        area_with_sram = area
...
class _configurabledecoderinvmux(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if use_lp_transistor == 0:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_configurabledecoderi(
                subcircuit_filename, self.name)
        else:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_configurabledecoderi_lp(
                subcircuit_filename, self.name)
    ...
    def generate_top(self):
        ...
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_configurabledecoderi_top(
                self.name,  self.numberofgates2, self.numberofgates3, self.ConfiDecodersize)
        else:
            self.top_spice_path = top_level.generate_configurabledecoderi_top_lp(
                self.name,  self.numberofgates2, self.numberofgates3, self.ConfiDecodersize)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        area = (area_dict["inv_xconfigurabledecoderi_1"] * self.ConfiDecodersize +
                2 * area_dict["tgate_xconfigurabledecoderi_2"]) * self.ConfiDecodersize
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers):
        ...
        wire_lengths["wire_" +
                     self.name] = width_dict["configurabledecoder_wodriver"]
        wire_layers["wire_" + self.name] = 0
...
class _rowdecoder0(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if use_lp_transistor == 0:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_rowdecoderstage0(
                subcircuit_filename, self.name)
        else:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_rowdecoderstage0_lp(
                subcircuit_filename, self.name)
    ...
    def generate_top(self):
        print("Generating row decoder initial stage")
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_rowdecoderstage0_top(
                self.name, self.numberofgates2, self.numberofgates3, self.decodersize, self.label2, self.label3)
        else:
            self.top_spice_path = top_level.generate_rowdecoderstage0_top_lp(
                self.name, self.numberofgates2, self.numberofgates3, self.decodersize, self.label2, self.label3)
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers):
        ...
        wire_lengths["wire_" + self.name] = math.sqrt(
            width_dict["decoder"]*width_dict["decoder"] + width_dict["wordline_total"]*width_dict["wordline_total"])
        wire_layers["wire_" + self.name] = 0
...
class _rowdecoder1(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if use_lp_transistor == 0:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_rowdecoderstage1(
                subcircuit_filename, self.name, self.nandtype)
        else:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_rowdecoderstage1_lp(
                subcircuit_filename, self.name, self.nandtype)

        self.initial_transistor_sizes["inv_nand" +
                                      str(self.nandtype) + "_" + self.name + "_1_nmos"] = 1
        self.initial_transistor_sizes["inv_nand" +
                                      str(self.nandtype) + "_" + self.name + "_1_pmos"] = 1
        self.initial_transistor_sizes["inv_" + self.name + "_2_nmos"] = 1
        self.initial_transistor_sizes["inv_" + self.name + "_2_pmos"] = 1
    ...
    def generate_top(self):
        ...
        pass
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_rowdecoderstage1_top(
                self.name, self.fanout, self.nandtype)
        else:
            self.top_spice_path = top_level.generate_rowdecoderstage1_top_lp(
                self.name, self.fanout, self.nandtype)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        area = (area_dict["inv_nand" + str(self.nandtype) + "_" + self.name +
                "_1"] + area_dict["inv_" + self.name + "_2"]) * self.areafac
        area_with_sram = area
        width = math.sqrt(area)
        width_with_sram = math.sqrt(area_with_sram)
        area_dict[self.name] = area
...
class _wordlinedriver(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        # Call generation function
        if use_lp_transistor == 0:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_wordline_driver(
                subcircuit_filename, self.name, self.number_of_banks + 1, self.wl_repeater)
        else:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_wordline_driver_lp(
                subcircuit_filename, self.name, self.number_of_banks + 1, self.wl_repeater)
        ...
        if self.number_of_banks == 1:
            self.initial_transistor_sizes["inv_nand2_" +
                                          self.name + "_1_nmos"] = 1
            self.initial_transistor_sizes["inv_nand2_" +
                                          self.name + "_1_pmos"] = 1
            self.initial_transistor_sizes["inv_" + self.name + "_2_nmos"] = 1
            ...
            self.initial_transistor_sizes["inv_nand3_" +
                                          self.name + "_1_nmos"] = 1
            self.initial_transistor_sizes["inv_nand3_" +
                                          self.name + "_1_pmos"] = 1
            self.initial_transistor_sizes["inv_" + self.name + "_2_nmos"] = 1
    ...
    def generate_top(self):
        ...
        pass
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_wordline_driver_top(
                self.name, self.rowsram, self.number_of_banks + 1, self.is_rowdecoder_2stage, self.wl_repeater)
        else:
            self.top_spice_path = top_level.generate_wordline_driver_top_lp(
                self.name, self.rowsram, self.number_of_banks + 1, self.is_rowdecoder_2stage, self.wl_repeater)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        area = area_dict["inv_" + self.name + "_3"] + \
            area_dict["inv_" + self.name + "_4"]
        if self.wl_repeater == 1:
            area *= 2
        if self.number_of_banks == 1:
            area += area_dict["inv_nand2_" + self.name +
                              "_1"]*2 + area_dict["inv_" + self.name + "_2"]
        else:
            area += area_dict["inv_nand3_" + self.name +
                              "_1"]*3 + area_dict["inv_" + self.name + "_2"]
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers):
        ...
        # Update wire lengths
        wire_lengths["wire_" +
                     self.name] = wire_lengths["wire_memorycell_horizontal"]
...
class _rowdecoderstage3(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        # Call generation function
        if use_lp_transistor == 0:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_rowdecoderstage3(
                subcircuit_filename, self.name, self.fanout, self.gatesize - 1)
        else:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_rowdecoderstage3(
                subcircuit_filename, self.name, self.fanout, self.gatesize - 1)
        # Initialize transistor sizes (to something more reasonable than all min size, but not necessarily a good choice, depends on architecture params)
        self.initial_transistor_sizes["inv_nand" +
                                      str(self.fanout) + "_" + self.name + "_1_nmos"] = 1
        self.initial_transistor_sizes["inv_nand" +
                                      str(self.fanout) + "_" + self.name + "_1_pmos"] = 1
        self.initial_transistor_sizes["inv_" + self.name + "_2_nmos"] = 1
        self.initial_transistor_sizes["inv_" + self.name + "_2_pmos"] = 1
        if self.gatesize == 3:
            self.initial_transistor_sizes["inv_nand2_" +
    ...
    def generate_top(self):
        ...
        pass
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_rowdecoderstage3_top(
                self.name, self.fanin1, self.fanin2, self.rowsram, self.gatesize - 1, self.fanout)
        else:
            self.top_spice_path = top_level.generate_rowdecoderstage3_top_lp(
                self.name, self.fanin1, self.fanin2, self.rowsram, self.gatesize - 1, self.fanout)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        area = (area_dict["inv_nand" + str(self.fanout) + "_" + self.name + "_1"] + area_dict["inv_" +
                self.name + "_2"] + area_dict["inv_" + self.name + "_5"] + area_dict["inv_" + self.name + "_6"])
        if self.gatesize == 3:
            area += area_dict["inv_nand2_" + self.name +
                              "_3"]*2 + area_dict["inv_" + self.name + "_4"]
        else:
            area += area_dict["inv_nand3_" + self.name +
                              "_3"]*3 + area_dict["inv_" + self.name + "_4"]
...
class _powermtjread(_SizableCircuit):
    ...
    def generate_top(self):
        ...
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_mtj_read_power_top(
                self.name, self.SRAM_per_column)
        else:
            self.top_spice_path = top_level.generate_mtj_read_power_top_lp(
                self.name, self.SRAM_per_column)
...
class _powermtjwrite(_SizableCircuit):
    ...
    def generate_top(self):
        ...
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_mtj_write_power_top(
                self.name, self.SRAM_per_column)
        else:
            self.top_spice_path = top_level.generate_mtj_write_power_top_lp(
                self.name, self.SRAM_per_column)
...
class _powersramwritehh(_SizableCircuit):
    ...
    def generate_top(self):
        ...
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_sram_writehh_power_top(
                self.name, self.SRAM_per_column, self.column_multiplexity - 1)
        else:
            self.top_spice_path = top_level.generate_sram_writehh_power_top_lp(
                self.name, self.SRAM_per_column, self.column_multiplexity - 1)
...
class _powersramwritep(_SizableCircuit):
    ...
    def generate_top(self):
        print("Generating top level module to measure SRAM write power")
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_sram_writep_power_top(
                self.name, self.SRAM_per_column, self.column_multiplexity - 1)
        else:
            self.top_spice_path = top_level.generate_sram_writep_power_top_lp(
                self.name, self.SRAM_per_column, self.column_multiplexity - 1)
...
class _powersramwritelh(_SizableCircuit):
    ...
    def generate_top(self):
        print("Generating top level module to measure SRAM write power")
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_sram_writelh_power_top(
                self.name, self.SRAM_per_column, self.column_multiplexity - 1)
        else:
            self.top_spice_path = top_level.generate_sram_writelh_power_top_lp(
                self.name, self.SRAM_per_column, self.column_multiplexity - 1)
...
class _powersramread(_SizableCircuit):
    ...
    def generate_top(self):
        ...
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_sram_read_power_top(
                self.name, self.SRAM_per_column, self.column_multiplexity - 1)
        else:
            self.top_spice_path = top_level.generate_sram_read_power_top_lp(
                self.name, self.SRAM_per_column, self.column_multiplexity - 1)
...
class _columndecoder(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if use_lp_transistor == 0:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_columndecoder(
                subcircuit_filename, self.name, self.col_decoder_bitssize)
        else:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_columndecoder_lp(
                subcircuit_filename, self.name, self.col_decoder_bitssize)
    ...
    def generate_top(self):
        ...
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_columndecoder_top(
                self.name,  self.numberoftgates, self.col_decoder_bitssize)
        else:
            self.top_spice_path = top_level.generate_columndecoder_top_lp(
                self.name,  self.numberoftgates, self.col_decoder_bitssize)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        area = area_dict["inv_columndecoder_1"] * self.col_decoder_bitssize + area_dict["inv_columndecoder_2"] * \
            self.col_decoder_bitssize * 2**self.col_decoder_bitssize + \
            area_dict["inv_columndecoder_3"] * 2**self.col_decoder_bitssize
        area_with_sram = area
        width = math.sqrt(area)
        width_with_sram = math.sqrt(area_with_sram)
        area_dict[self.name] = area
...
class _writedriver(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if use_lp_transistor == 0:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_writedriver(
                subcircuit_filename, self.name)
        else:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_writedriver_lp(
                subcircuit_filename, self.name)
    ...
    def generate_top(self):
        ...
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_writedriver_top(
                self.name, self.numberofsramsincol)
        else:
            self.top_spice_path = top_level.generate_writedriver_top_lp(
                self.name, self.numberofsramsincol)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        area = area_dict["inv_writedriver_1"] + \
            area_dict["inv_writedriver_2"] + \
            area_dict["tgate_writedriver_3"] * 4
        area_with_sram = area
        width = math.sqrt(area)
        width_with_sram = math.sqrt(area_with_sram)
        area_dict[self.name] = area
...
class _samp(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if use_lp_transistor == 0:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_samp(
                subcircuit_filename, self.name)
        else:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_samp_lp(
                subcircuit_filename, self.name)
    ...
    def generate_top(self):
        print("Generating sense amplifier circuit")
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_samp_top_part1(
                self.name, self.numberofsramsincol, self.difference)
        else:
            self.top_spice_path = top_level.generate_samp_top_part1_lp(
                self.name, self.numberofsramsincol, self.difference)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        area = area_dict["inv_samp_output_1"] + area_dict["ptran_samp_output_1"]*2 + \
            area_dict["ptran_samp_output_2"]*2 + \
            area_dict["ptran_samp_output_3"]*2 + \
            area_dict["ptran_samp_output_1"]
        area_with_sram = area
        width = math.sqrt(area)
        width_with_sram = math.sqrt(area_with_sram)
        area_dict[self.name] = area
...
class _samp_part2(_SizableCircuit):
    ...
    def generate_top(self):
        ...
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_samp_top_part2(
                self.name, self.numberofsramsincol, self.difference)
        else:
            self.top_spice_path = top_level.generate_samp_top_part2_lp(
                self.name, self.numberofsramsincol, self.difference)
...
class _prechargeandeq(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if use_lp_transistor == 0:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_precharge(
                subcircuit_filename, self.name)
        else:
            self.transistor_names, self.wire_names = memory_subcircuits.generate_precharge_lp(
                subcircuit_filename, self.name)
    ...
    def generate_top(self):
        print("Generating precharge and equalization circuit")
        if use_lp_transistor == 0:
            self.top_spice_path = top_level.generate_precharge_top(
                self.name, self.numberofsrams)
        else:
            self.top_spice_path = top_level.generate_precharge_top_lp(
                self.name, self.numberofsrams)
    ...
    def update_area(self, area_dict, width_dict):
        ...
        area = area_dict["ptran_precharge_side"] * \
            2 + area_dict["ptran_equalization"]
        area_with_sram = area
        width = math.sqrt(area)
        width_with_sram = math.sqrt(area_with_sram)
        area_dict[self.name] = area
...
class _levelshifter(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename):
        ...
        self.transistor_names = []
        self.transistor_names.append(
            memory_subcircuits.generate_level_shifter(subcircuit_filename, self.name))
    ...
    def update_area(self, area_dict, width_dict):
        ...
        area_dict[self.name] = area_dict["inv_level_shifter_1"] * 3 + \
            area_dict["ptran_level_shifter_2"] * 2 + \
            area_dict["ptran_level_shifter_3"] * 2
...
class _mtjsamp(_SizableCircuit):
    ...
    def generate_top(self):
        ...
        self.top_spice_path = top_level.generate_mtj_sa_top(
            self.name, self.colsize)
...
class _mtjblcharging(_SizableCircuit):
    ...
    def generate_top(self):
        ...
        self.top_spice_path = top_level.generate_mtj_charge(
            self.name, self.colsize)
...
class _mtjbldischarging(_SizableCircuit):
    ...
    def generate_top(self):
        ...
        self.top_spice_path = top_level.generate_mtj_discharge(
            self.name, self.colsize)
...
class _mtjbasiccircuits(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename):
        ...
        self.transistor_names = []
        self.transistor_names.append(
            memory_subcircuits.generate_mtj_sa_lp(subcircuit_filename, self.name))
        self.transistor_names.append(
            memory_subcircuits.generate_mtj_writedriver_lp(subcircuit_filename, self.name))
        self.transistor_names.append(
            memory_subcircuits.generate_mtj_cs_lp(subcircuit_filename, self.name))
    ...
    def update_area(self, area_dict, width_dict):
        ...
        area_dict[self.name + "_sa"] = 2 * (area_dict["ptran_mtj_subcircuits_mtjsa_1"] + area_dict["inv_mtj_subcircuits_mtjsa_2"] +
                                            area_dict["ptran_mtj_subcircuits_mtjsa_3"] + area_dict["ptran_mtj_subcircuits_mtjsa_4"] + area_dict["ptran_mtj_subcircuits_mtjsa_3"] * 2)
        area_dict[self.name +
                  "_sa"] += area_dict["inv_mtj_subcircuits_mtjsa_6"] * 2
        width_dict[self.name + "_sa"] = math.sqrt(area_dict[self.name + "_sa"])

        area_dict[self.name + "_writedriver"] = area_dict["inv_mtj_subcircuits_mtjwd_1"] + \
            area_dict["inv_mtj_subcircuits_mtjwd_2"] + \
            area_dict["inv_mtj_subcircuits_mtjwd_3"]
        width_dict[self.name +
                   "_writedriver"] = math.sqrt(area_dict[self.name + "_writedriver"])

        area_dict[self.name + "_cs"] = area_dict["tgate_mtj_subcircuits_mtjcs_1"] + \
            area_dict["ptran_mtj_subcircuits_mtjcs_0"]
...
class _memorycell(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        if self.memory_technology == "SRAM":
            if use_lp_transistor == 0:
                self.transistor_names, self.wire_names = memory_subcircuits.generate_memorycell(
                    subcircuit_filename, self.name)
            else:
                self.transistor_names, self.wire_names = memory_subcircuits.generate_memorycell_lp(
                    subcircuit_filename, self.name)
        else:
            memory_subcircuits.generate_mtj_memorycell_high_lp(
                subcircuit_filename, self.name)
            memory_subcircuits.generate_mtj_memorycell_low_lp(
                subcircuit_filename, self.name)
            memory_subcircuits.generate_mtj_memorycell_reference_lp(
                subcircuit_filename, self.name)
            memory_subcircuits.generate_mtj_memorycellh_reference_lp(
                subcircuit_filename, self.name)
            memory_subcircuits.generate_mtj_memorycell_reference_lp_target(
                subcircuit_filename, self.name)
    ...
    def update_area(self, area_dict, width_dict):
        """ Update area. To do this, we use area_dict which is a dictionary, maintained externally, that contains
            ...
            We update area_dict and width_dict with calculations performed in this function. """
        if self.memory_technology == "SRAM":
            area = area_dict["ramsram"] * self.RAMheight * \
                self.RAMwidth * self.number_of_banks
        else:
            area = area_dict["rammtj"] * self.RAMheight * \
                self.RAMwidth * self.number_of_banks
        area_with_sram = area
        width = math.sqrt(area)
        width_with_sram = math.sqrt(area_with_sram)
...
class _RAMLocalRoutingWireLoad:
    ...
    def __init__(self, row_decoder_bits, col_decoder_bits, conf_decoder_bits):
        ...
        self.RAM_input_usage_assumption = float((2 + 2*(row_decoder_bits + col_decoder_bits) + 2 ** (conf_decoder_bits))/(
            2 + 2*(row_decoder_bits + col_decoder_bits + conf_decoder_bits) + 2 ** (conf_decoder_bits)))
        # Total number of local mux inputs per wire
        self.mux_inputs_per_wire = -1
        # Number of on inputs connected to each wire
        self.on_inputs_per_wire = -1
    ...
    def generate(self, subcircuit_filename, specs, RAM_local_mux):
        ...
        # Generate SPICE deck
        self.wire_names = load_subcircuits.RAM_local_routing_load_generate(
            subcircuit_filename, self.on_inputs_per_wire, self.partial_inputs_per_wire, self.off_inputs_per_wire)
    ...
    def _compute_load(self, specs, RAM_local_mux):
        ...
        num_local_routing_wires = (2 + 2*(self.row_decoder_bits + self.col_decoder_bits +
                                   self.conf_decoder_bits) + 2 ** (self.conf_decoder_bits))
        self.mux_inputs_per_wire = RAM_local_mux.implemented_size

        # Now we compute how many "on" inputs are connected to each routing wire
        # This is a funtion of lut input usage, number of lut inputs and number of local routing wires
        num_local_muxes_used = self.RAM_input_usage_assumption * \
            (2 + 2*(self.row_decoder_bits + self.col_decoder_bits +
             self.conf_decoder_bits) + 2 ** (self.conf_decoder_bits))
        self.on_inputs_per_wire = int(
            num_local_muxes_used/num_local_routing_wires)
        # We want to model for the case where at least one "on" input is connected to the local wire, so make sure it's at least 1
        ...
        # We can figure out the number of muxes used by using the "on" assumption and the number of local routing wires.
        self.partial_inputs_per_wire = int(
            (RAM_local_mux.level2_size - 1.0)*num_local_muxes_used/num_local_routing_wires)
        # Make it at least 1
        if self.partial_inputs_per_wire < 1:
            self.partial_inputs_per_wire = 1
...
class _RAM(_CompoundCircuit):
    ...
    def __init__(self, row_decoder_bits, col_decoder_bits, conf_decoder_bits, RAM_local_mux_size_required, RAM_num_local_mux_per_tile, use_tgate, sram_area, number_of_banks, memory_technology, cspecs, process_data_filename, read_to_write_ratio):
        ...
        # Create local mux object
        self.RAM_local_mux = _RAMLocalMUX(
            RAM_local_mux_size_required, RAM_num_local_mux_per_tile, use_tgate)
        # Create the local routing wire load module
        self.RAM_local_routing_wire_load = _RAMLocalRoutingWireLoad(
            self.row_decoder_bits, col_decoder_bits, conf_decoder_bits)
        # Create memory cells
        self.memorycells = _memorycell(self.use_tgate, 2**(conf_decoder_bits + col_decoder_bits), 2**(
            self.row_decoder_bits), sram_area, self.number_of_banks, self.memory_technology)
        # calculat the number of input ports for the ram module
        self.ram_inputs = (3 + 2*(self.row_decoder_bits + col_decoder_bits +
                           number_of_banks) + 2 ** (self.conf_decoder_bits))
        ...
        self.rowdecoder_stage0 = _rowdecoder0(
            use_tgate, self.count_small_row_dec_nand3, 0, self.count_small_row_dec_nand2, 0, self.row_decoder_bits)
        ...
        self.wordlinedriver = _wordlinedriver(use_tgate, 2**(self.col_decoder_bits + self.conf_decoder_bits),
                                              self.number_of_banks, 2**self.row_decoder_bits, 1, self.memory_technology)
        ...
        if self.predecoder1 == 2 or self.predecoder2 == 2 or self.predecoder3 == 2:
            self.rowdecoder_stage1_size2 = _rowdecoder1(
                use_tgate, 2 ** (self.row_decoder_bits - 2), self.fanouttypeforrowdec, 2, self.area2)
        ...
        if self.predecoder1 == 3 or self.predecoder2 == 3 or self.predecoder3 == 3:
            self.rowdecoder_stage1_size3 = _rowdecoder1(
                use_tgate, 2 ** (self.row_decoder_bits - 3), self.fanouttypeforrowdec, 3, self.area3)
        ...
        self.rowdecoder_stage3 = _rowdecoderstage3(use_tgate, self.area2, self.area3, 2**(
            self.col_decoder_bits + self.conf_decoder_bits), self.gatesize_stage3, self.fanouttypeforrowdec, 2 ** self.row_decoder_bits)

        # measure memory core power:
        if self.memory_technology == "SRAM":
            self.power_sram_read = _powersramread(
                2**self.row_decoder_bits, 2**self.col_decoder_bits)
            self.power_sram_writelh = _powersramwritelh(
                2**self.row_decoder_bits, 2**self.col_decoder_bits)
            self.power_sram_writehh = _powersramwritehh(
                2**self.row_decoder_bits, 2**self.col_decoder_bits)
            self.power_sram_writep = _powersramwritep(
                2**self.row_decoder_bits, 2**self.col_decoder_bits)
        ...
        if self.memory_technology == "SRAM":
            self.precharge = _prechargeandeq(
                self.use_tgate, 2**self.row_decoder_bits)
            self.samp_part2 = _samp_part2(
                self.use_tgate, 2**self.row_decoder_bits, 0.3)
            self.samp = _samp(self.use_tgate, 2**self.row_decoder_bits, 0, 0.3)
            self.writedriver = _writedriver(
                self.use_tgate, 2**self.row_decoder_bits)
        ...
        self.columndecoder = _columndecoder(
            self.use_tgate, 2**(self.col_decoder_bits + self.conf_decoder_bits), self.col_decoder_bits)
        ...
        if self.cpredecoder1 == 3 or self.cpredecoder2 == 3 or self.cpredecoder3 == 3:
            self.configurabledecoder3ii = _configurabledecoder3ii(
                use_tgate, 3, 2**(self.cpredecoder1+self.cpredecoder1+self.cpredecoder1 - 3), self.cfanouttypeconf, self.stage1output3)
            self.cfanin1 = 2**(self.cpredecoder1 +
                               self.cpredecoder2+self.cpredecoder3 - 3)
            self.cvalidobj1 = 1

        if self.cpredecoder2 == 2 or self.cpredecoder2 == 2 or self.cpredecoder3 == 2:
            self.configurabledecoder2ii = _configurabledecoder2ii(
                use_tgate, 2, 2**(self.cpredecoder1+self.cpredecoder1+self.cpredecoder1 - 2), self.cfanouttypeconf, self.stage1output2)
            self.cfanin2 = 2**(self.cpredecoder1 +
                               self.cpredecoder2+self.cpredecoder3 - 2)
            self.cvalidobj2 = 1

        self.configurabledecoderi = _configurabledecoderinvmux(
            use_tgate, self.stage1output2/2, self.stage1output3/2, self.conf_decoder_bits)
        self.configurabledecoderiii = _configurabledecoderiii(
            use_tgate, self.cfanouttypeconf, self.cfanin1, self.cfanin2, 2**self.conf_decoder_bits)

        self.pgateoutputcrossbar = _pgateoutputcrossbar(
            2**self.conf_decoder_bits)

    def generate(self, subcircuits_filename, min_tran_width, specs):
        print("Generating RAM block")
        init_tran_sizes = {}
        init_tran_sizes.update(self.RAM_local_mux.generate(
            subcircuits_filename, min_tran_width))

        if self.valid_row_dec_size2 == 1:
            init_tran_sizes.update(self.rowdecoder_stage1_size2.generate(
                subcircuits_filename, min_tran_width))

        if self.valid_row_dec_size3 == 1:
            init_tran_sizes.update(self.rowdecoder_stage1_size3.generate(
                subcircuits_filename, min_tran_width))

        init_tran_sizes.update(self.rowdecoder_stage3.generate(
            subcircuits_filename, min_tran_width))

        init_tran_sizes.update(self.rowdecoder_stage0.generate(
            subcircuits_filename, min_tran_width))
        init_tran_sizes.update(self.wordlinedriver.generate(
            subcircuits_filename, min_tran_width))
        self.RAM_local_routing_wire_load.generate(
            subcircuits_filename, specs, self.RAM_local_mux)

        self.memorycells.generate(subcircuits_filename, min_tran_width)

        if self.memory_technology == "SRAM":
            init_tran_sizes.update(self.precharge.generate(
                subcircuits_filename, min_tran_width))
            self.samp.generate(subcircuits_filename, min_tran_width)
            init_tran_sizes.update(self.writedriver.generate(
                subcircuits_filename, min_tran_width))
        else:
            init_tran_sizes.update(self.bldischarging.generate(
                subcircuits_filename, min_tran_width))
            init_tran_sizes.update(
                self.mtjbasics.generate(subcircuits_filename))

        init_tran_sizes.update(self.levelshift.generate(subcircuits_filename))

        init_tran_sizes.update(self.columndecoder.generate(
            subcircuits_filename, min_tran_width))

        init_tran_sizes.update(self.configurabledecoderi.generate(
            subcircuits_filename, min_tran_width))

        init_tran_sizes.update(self.configurabledecoderiii.generate(
            subcircuits_filename, min_tran_width))

        if self.cvalidobj1 == 1:
            init_tran_sizes.update(self.configurabledecoder3ii.generate(
                subcircuits_filename, min_tran_width))
        if self.cvalidobj2 == 1:
            init_tran_sizes.update(self.configurabledecoder2ii.generate(
                subcircuits_filename, min_tran_width))

        init_tran_sizes.update(self.pgateoutputcrossbar.generate(
            subcircuits_filename, min_tran_width))
    ...
    def _update_process_data(self):
        ...
        process_data_file.write(".PARAM supply_v = " +
                                str(self.cspecs.vdd) + "\n")
        process_data_file.write(".PARAM sram_v = " +
                                str(self.cspecs.vsram) + "\n")
        process_data_file.write(".PARAM sram_n_v = " +
                                str(self.cspecs.vsram_n) + "\n")
        process_data_file.write(".PARAM Rcurrent = " +
                                str(self.cspecs.worst_read_current) + "\n")
        process_data_file.write(
            ".PARAM supply_v_lp = " + str(self.cspecs.vdd_low_power) + "\n\n")

        if use_lp_transistor == 0:
            process_data_file.write(
                ".PARAM sense_v = " + str(self.cspecs.vdd - self.cspecs.sense_dv) + "\n\n")
        else:
            process_data_file.write(
                ".PARAM sense_v = " + str(self.cspecs.vdd_low_power - self.cspecs.sense_dv) + "\n\n")

        process_data_file.write(
            ".PARAM mtj_worst_high = " + str(self.cspecs.MTJ_Rhigh_worstcase) + "\n")
        process_data_file.write(
            ".PARAM mtj_worst_low = " + str(self.cspecs.MTJ_Rlow_worstcase) + "\n")
        process_data_file.write(
            ".PARAM mtj_nominal_low = " + str(self.cspecs.MTJ_Rlow_nominal) + "\n\n")
        process_data_file.write(
            ".PARAM mtj_nominal_high = " + str(6250) + "\n\n")
        process_data_file.write(
            ".PARAM vref = " + str(self.cspecs.vref) + "\n")
        process_data_file.write(
            ".PARAM vclmp = " + str(self.cspecs.vclmp) + "\n")

        process_data_file.write("* Misc parameters\n")

        process_data_file.write(
            ".PARAM ram_frequency = " + str(self.frequency) + "\n")
        process_data_file.write(
            ".PARAM precharge_max = " + str(self.T1) + "\n")
        if self.cspecs.memory_technology == "SRAM":
            process_data_file.write(
                ".PARAM wl_eva = " + str(self.T1 + self.T2) + "\n")
            process_data_file.write(
                ".PARAM sa_xbar_ff = " + str(self.frequency) + "\n")
        elif self.cspecs.memory_technology == "MTJ":
            process_data_file.write(
                ".PARAM target_bl = " + str(self.target_bl) + "\n")
            process_data_file.write(
                ".PARAM time_bl = " + str(self.blcharging.delay) + "\n")
        ...
        process_data_file.write(
            ".PARAM gate_length = " + str(self.cspecs.gate_length) + "n\n")
        process_data_file.write(
            ".PARAM trans_diffusion_length = " + str(self.cspecs.trans_diffusion_length) + "n\n")
        process_data_file.write(
            ".PARAM min_tran_width = " + str(self.cspecs.min_tran_width) + "n\n")
        process_data_file.write(
            ".param rest_length_factor=" + str(self.cspecs.rest_length_factor) + "\n")
        process_data_file.write("\n")
        ...
        process_data_file.write(
            ".LIB \"" + self.cspecs.model_path + "\" " + self.cspecs.model_library + "\n\n")
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers):
        ...
        self.RAM_local_routing_wire_load.update_wires(
            width_dict, wire_lengths, wire_layers)
        self.rowdecoder_stage0.update_wires(
            width_dict, wire_lengths, wire_layers)
        ...
        self.configurabledecoderi.update_wires(
            width_dict, wire_lengths, wire_layers)
        self.pgateoutputcrossbar.update_wires(
            width_dict, wire_lengths, wire_layers)
        self.configurabledecoderiii.update_wires(
            width_dict, wire_lengths, wire_layers)
...
class _HBLocalMUX(_SizableCircuit):
    ...
    def generate_top(self):
        ...
        self.top_spice_path = top_level.generate_HB_local_mux_top(
            self.name, self.hb_parameters['name'])
...
class _HBLocalRoutingWireLoad:
    ...
    def generate(self, subcircuit_filename, specs, HB_local_mux):
        ...
        # Generate SPICE deck
        self.wire_names = load_subcircuits.hb_local_routing_load_generate(
            subcircuit_filename, self.on_inputs_per_wire, self.partial_inputs_per_wire, self.off_inputs_per_wire, self.name, HB_local_mux.name)

    def update_wires(self, width_dict, wire_lengths, wire_layers):
        """ Update wire lengths and wire layers based on the width of things, obtained from width_dict. """

        # Update wire lengths
        wire_lengths["wire_"+self.hb_parameters['name'] +
                     "_local_routing"] = width_dict[self.hb_parameters['name']]
        # Update wire layers
    ...
    def _compute_load(self, specs, HB_local_mux):
        ...
        num_local_muxes_used = self.RAM_input_usage_assumption * \
            self.hb_parameters['num_gen_inputs']
        self.on_inputs_per_wire = int(
            num_local_muxes_used/num_local_routing_wires)
        # We want to model for the case where at least one "on" input is connected to the local wire, so make sure it's at least 1
        if self.on_inputs_per_wire < 1:
        ...
        # We can figure out the number of muxes used by using the "on" assumption and the number of local routing wires.
        self.partial_inputs_per_wire = int(
            (HB_local_mux.level2_size - 1.0)*num_local_muxes_used/num_local_routing_wires)
        # Make it at least 1
        if self.partial_inputs_per_wire < 1:
            self.partial_inputs_per_wire = 1
...
class _dedicated_routing_driver(_SizableCircuit):
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        self.transistor_names, self.wire_names = mux_subcircuits.generate_dedicated_driver(
            subcircuit_filename, self.name, self.num_buffers, self.top_name)

        for i in range(1, self.num_buffers * 2 + 1):
            self.initial_transistor_sizes["inv_" +
                                          self.name + "_"+str(i)+"_nmos"] = 2
            self.initial_transistor_sizes["inv_" +
                                          self.name + "_"+str(i)+"_pmos"] = 2
    ...
    def generate_top(self):
        ...
        self.top_spice_path = top_level.generate_dedicated_driver_top(
            self.name, self.top_name, self.num_buffers)
...
class _hard_block(_CompoundCircuit):
    ...
    def __init__(self, filename, use_tgate):
        ...
        self.mux = _HBLocalMUX(int(math.ceil(self.parameters['num_gen_inputs']/self.parameters['num_crossbars'] *
                               self.parameters['crossbar_population'])), self.parameters['num_gen_inputs'], use_tgate, self.parameters)
        ...
        if self.parameters['num_dedicated_outputs'] > 0:
            ...
            self.dedicated = _dedicated_routing_driver(
                self.name + "_ddriver", self.name, 2)
    ...
    def generate(self, subcircuit_filename, min_tran_width):
        ...
        # generate subblocks
        init_tran_sizes = {}
        init_tran_sizes.update(self.mux.generate(
            subcircuit_filename, min_tran_width))
        if self.parameters['num_dedicated_outputs'] > 0:
            init_tran_sizes.update(self.dedicated.generate(
                subcircuit_filename, min_tran_width))
        # wireload
    ...
    def generate_top(self):
        ...
        self.area = self.flow_results[0] * \
            self.parameters['area_scale_factor'] * (1e+6)

        self.mux.lowerbounddelay = self.flow_results[1] * (
            1.0/self.parameters['freq_scale_factor']) * 1e-9

        if self.parameters['num_dedicated_outputs'] > 0:
            self.dedicated.lowerbounddelay = self.flow_results[1] * (
                1.0/self.parameters['freq_scale_factor']) * 1e-9
    ...
    def update_area(self, area_dict, width_dict):
        ...
        if self.parameters['num_dedicated_outputs'] > 0:
            area = self.parameters['num_dedicated_outputs'] * area_dict[self.name + "_ddriver"] + \
                self.parameters['num_gen_inputs'] * \
                area_dict[self.mux.name] + self.area
            area_with_sram = self.parameters['num_dedicated_outputs'] * area_dict[self.name + "_ddriver"] + \
                self.parameters['num_gen_inputs'] * \
                area_dict[self.mux.name + "_sram"] + self.area
        else:
            area = self.parameters['num_gen_inputs'] * \
                area_dict[self.mux.name] + self.area
            area_with_sram = self.parameters['num_gen_inputs'] * \
                area_dict[self.mux.name + "_sram"] + self.area
    ...
    def update_wires(self, width_dict, wire_lengths, wire_layers):
        ...
        wire_lengths["wire_" + self.name + "_2"] = width_dict["tile"] * \
            self.parameters['soft_logic_per_block']
        wire_lengths["wire_" + self.name +
                     "_local_routing_wire_load"] = width_dict[self.name]
    ...
    def print_details(self, report_file):
        ...
        utils.print_and_write(
            report_file, "  DETAILS OF HARD BLOCK: " + self.name)
...
class FPGA:
    ...
    def __init__(self, arch_params_dict, run_options, spice_interface):
        ...
        self.sb_mux = _SwitchBlockMUX(
            sb_mux_size_required, num_sb_mux_per_tile, self.specs.use_tgate)
        ...
        self.cb_mux = _ConnectionBlockMUX(
            cb_mux_size_required, num_cb_mux_per_tile, self.specs.use_tgate)
        ...
        local_mux_size_required = int(
            (self.specs.I + self.specs.num_ble_local_outputs*self.specs.N) * self.specs.Fclocal)
        num_local_mux_per_tile = self.specs.N * \
            (self.specs.K+self.specs.independent_inputs)
        ...
        if self.specs.enable_carry_chain == 1 and self.specs.carry_chain_type == "skip":
            self.carry_skip_periphery_count = int(math.floor(
                (self.specs.N * self.specs.FAs_per_flut)/self.skip_size))
        ...
        if self.specs.enable_carry_chain == 1:
            self.carrychain = _CarryChain(
                self.specs.use_finfet, self.specs.carry_chain_type, self.specs.N, self.specs.FAs_per_flut)
            self.carrychainperf = _CarryChainPer(
                self.specs.use_finfet, self.specs.carry_chain_type, self.specs.N, self.specs.FAs_per_flut, self.specs.use_tgate)
            self.carrychainmux = _CarryChainMux(
                self.specs.use_finfet, self.specs.use_fluts, self.specs.use_tgate)
            self.carrychaininter = _CarryChainInterCluster(
                self.specs.use_finfet, self.specs.carry_chain_type, inter_wire_length)
            if self.specs.carry_chain_type == "skip":
                self.carrychainand = _CarryChainSkipAnd(
                    self.specs.use_finfet, self.specs.use_tgate, self.specs.carry_chain_type, self.specs.N, self.specs.FAs_per_flut, self.skip_size)
                self.carrychainskipmux = _CarryChainSkipMux(
                    self.specs.use_finfet, self.specs.carry_chain_type, self.specs.use_tgate)
        ...
        RAM_num_mux_per_tile = (3 + 2*(self.specs.row_decoder_bits + self.specs.col_decoder_bits +
                                self.specs.conf_decoder_bits) + 2 ** (self.specs.conf_decoder_bits))
        self.RAM = _RAM(self.specs.row_decoder_bits, self.specs.col_decoder_bits, self.specs.conf_decoder_bits, RAM_local_mux_size_required,
                        RAM_num_mux_per_tile, self.specs.use_tgate, self.specs.sram_cell_area *
                        self.specs.min_width_tran_area, self.specs.number_of_banks,
    ...
    def generate(self, is_size_transistors):
        ...
        self.transistor_sizes.update(self.logic_cluster.generate(self.subcircuits_filename,
                                                                 self.specs.min_tran_width,
                                                                 self.specs))
        self.cluster_output_load.generate(
            self.subcircuits_filename, self.specs, self.sb_mux)
        self.routing_wire_load.generate(
            self.subcircuits_filename, self.specs, self.sb_mux, self.cb_mux)

        if self.specs.enable_carry_chain == 1:
            self.transistor_sizes.update(self.carrychain.generate(
                self.subcircuits_filename, self.specs.min_tran_width, self.specs.use_finfet))
            self.transistor_sizes.update(self.carrychainperf.generate(
                self.subcircuits_filename, self.specs.min_tran_width, self.specs.use_finfet))
            self.transistor_sizes.update(self.carrychainmux.generate(
                self.subcircuits_filename, self.specs.min_tran_width, self.specs.use_finfet))
            self.transistor_sizes.update(self.carrychaininter.generate(
                self.subcircuits_filename, self.specs.min_tran_width, self.specs.use_finfet))
            if self.specs.carry_chain_type == "skip":
                self.transistor_sizes.update(self.carrychainand.generate(
                    self.subcircuits_filename, self.specs.min_tran_width, self.specs.use_finfet))
                self.transistor_sizes.update(self.carrychainskipmux.generate(
                    self.subcircuits_filename, self.specs.min_tran_width, self.specs.use_finfet))

        if self.specs.enable_bram_block == 1:
            self.transistor_sizes.update(self.RAM.generate(
                self.subcircuits_filename, self.specs.min_tran_width, self.specs))

        for hardblock in self.hardblocklist:
            self.transistor_sizes.update(hardblock.generate(
                self.subcircuits_filename, self.specs.min_tran_width))
    ...
    def update_area(self):
        ...
        self.area_dict["sram"] = self.specs.sram_cell_area * \
            self.specs.min_width_tran_area
        ...
        if self.specs.enable_carry_chain == 1:
            ...
            if self.specs.carry_chain_type == "skip":
                ...
                self.carrychainskipmux.update_area(
                    self.area_dict, self.width_dict)
        ...
        switch_block_area = self.sb_mux.num_per_tile * \
            self.area_dict[self.sb_mux.name + "_sram"]
        ...
        connection_block_area = self.cb_mux.num_per_tile * \
            self.area_dict[self.cb_mux.name + "_sram"]
        ...
        if self.lb_height == 0.0:
            ...
            local_mux_area = self.logic_cluster.local_mux.num_per_tile * \
                self.area_dict[self.logic_cluster.local_mux.name + "_sram"]
            ...
            ff_area = self.specs.N * \
                self.area_dict[self.logic_cluster.ble.ff.name]
            ...
            sb_area_total = self.sb_mux.num_per_tile * \
                self.area_dict[self.sb_mux.name]
            sb_area_sram = self.sb_mux.num_per_tile * \
                self.area_dict[self.sb_mux.name + "_sram"] - sb_area_total
            cb_area_total = self.cb_mux.num_per_tile * \
                self.area_dict[self.cb_mux.name]
            cb_area_total_sram = self.cb_mux.num_per_tile * \
                self.area_dict[self.cb_mux.name + "_sram"] - cb_area_total

            local_mux_area = self.logic_cluster.local_mux.num_per_tile * \
                self.area_dict[self.logic_cluster.local_mux.name]
            local_mux_sram_area = self.logic_cluster.local_mux.num_per_tile * \
                (self.area_dict[self.logic_cluster.local_mux.name + "_sram"] -
                 self.area_dict[self.logic_cluster.local_mux.name])

            lut_area = self.specs.N * \
                self.area_dict["lut_and_drivers"] - self.specs.N * \
                (2**self.specs.K)*self.area_dict["sram"]
            lut_area_sram = self.specs.N * \
                (2**self.specs.K)*self.area_dict["sram"]

            ffableout_area_total = self.specs.N * \
                self.area_dict[self.logic_cluster.ble.ff.name]
            if self.specs.use_fluts:
                ffableout_area_total = 2 * ffableout_area_total
            ffableout_area_total = ffableout_area_total + \
                self.specs.N*(self.area_dict["ble_output"])

            cc_area_total = 0.0
            skip_size = 5
            self.carry_skip_periphery_count = int(math.floor(
                (self.specs.N * self.specs.FAs_per_flut)/skip_size))
            if self.specs.enable_carry_chain == 1:
                if self.carry_skip_periphery_count == 0 or self.specs.carry_chain_type == "ripple":
                    cc_area_total = self.specs.N * (self.area_dict["carry_chain"] * self.specs.FAs_per_flut + (
                        self.specs.FAs_per_flut) * self.area_dict["carry_chain_mux"])
                else:
                    cc_area_total = self.specs.N*(self.area_dict["carry_chain"] * self.specs.FAs_per_flut + (
                        self.specs.FAs_per_flut) * self.area_dict["carry_chain_mux"])
                    cc_area_total = cc_area_total + \
                        ((self.area_dict["xcarry_chain_and"] +
                         self.area_dict["xcarry_chain_mux"]) * self.carry_skip_periphery_count)
                cc_area_total = cc_area_total + \
                    self.area_dict["carry_chain_inter"]

            cluster_area = local_mux_area + local_mux_sram_area + \
                ffableout_area_total + cc_area_total + lut_area + lut_area_sram
            ...
            self.area_dict["local_mux_total"] = local_mux_area + \
                local_mux_sram_area
            self.width_dict["local_mux_total"] = math.sqrt(
                local_mux_area + local_mux_sram_area)

            self.area_dict["lut_total"] = lut_area + \
                self.specs.N*(2**self.specs.K)*self.area_dict["sram"]
            self.width_dict["lut_total"] = math.sqrt(
                lut_area + self.specs.N*(2**self.specs.K)*self.area_dict["sram"])

            self.area_dict["ff_total"] = self.specs.N * \
                self.area_dict[self.logic_cluster.ble.ff.name]
            self.width_dict["ff_total"] = math.sqrt(
                self.specs.N*self.area_dict[self.logic_cluster.ble.ff.name])

            self.area_dict["ffableout_area_total"] = ffableout_area_total
            self.width_dict["ffableout_area_total"] = math.sqrt(
                ffableout_area_total)

            self.area_dict["ble_output_total"] = self.specs.N * \
                (self.area_dict["ble_output"])
            self.width_dict["ble_output_total"] = math.sqrt(
                self.specs.N*(self.area_dict["ble_output"]))
        ...
        if self.specs.enable_carry_chain == 1:
            ...
            carry_chain_area = self.specs.N*(self.specs.FAs_per_flut * self.area_dict["carry_chain"] + (
                self.specs.FAs_per_flut) * self.area_dict["carry_chain_mux"]) + self.area_dict["carry_chain_inter"]
            if self.specs.carry_chain_type == "skip":
                self.carry_skip_periphery_count = int(math.floor(
                    (self.specs.N * self.specs.FAs_per_flut)/self.skip_size))
                carry_chain_area += self.carry_skip_periphery_count * \
                    (self.area_dict["xcarry_chain_and"] +
                     self.area_dict["xcarry_chain_mux"])
        ...
        if self.specs.enable_bram_block == 1:
            ...
            # LOCAL MUX + FF area
            RAM_local_mux_area = self.RAM.RAM_local_mux.num_per_tile * \
                self.area_dict[self.RAM.RAM_local_mux.name + "_sram"] + \
                self.area_dict[self.logic_cluster.ble.ff.name]
            self.area_dict["ram_local_mux_total"] = RAM_local_mux_area
            self.width_dict["ram_local_mux_total"] = math.sqrt(
                RAM_local_mux_area)

            # SB and CB in the RAM tile:
            RAM_area = (RAM_local_mux_area + self.area_dict[self.cb_mux.name + "_sram"] * self.RAM.ram_inputs + (
                2 ** (self.RAM.conf_decoder_bits + 3)) * self.area_dict[self.sb_mux.name + "_sram"])
            RAM_SB_area = 2 ** (self.RAM.conf_decoder_bits + 3) * \
                self.area_dict[self.sb_mux.name + "_sram"]
            RAM_CB_area = self.area_dict[self.cb_mux.name +
                                         "_sram"] * self.RAM.ram_inputs

            self.area_dict["level_shifters"] = self.area_dict["level_shifter"] * \
                self.RAM.RAM_local_mux.num_per_tile
            ...
            if self.RAM.memory_technology == "SRAM":
                ...
                self.area_dict["precharge_total"] = (self.area_dict[self.RAM.precharge.name] * 2 * (
                    2**(self.RAM.conf_decoder_bits+self.RAM.col_decoder_bits))) * self.number_of_banks
                ...
                if self.RAM.row_decoder_bits == 9:
                    self.area_dict["precharge_total"] = 2 * \
                        self.area_dict["precharge_total"]
                self.area_dict["samp_total"] = self.area_dict[self.RAM.samp.name] * \
                    2 * 2**(self.RAM.conf_decoder_bits) * self.number_of_banks
                self.area_dict["writedriver_total"] = self.area_dict[self.RAM.writedriver.name] * \
                    2 * 2**(self.RAM.conf_decoder_bits) * self.number_of_banks
                RAM_area += (self.area_dict["precharge_total"] +
                             self.area_dict["samp_total"] + self.area_dict["writedriver_total"])
                self.area_dict["columndecoder_total"] = ((self.area_dict["ramtgate"] * 4 * (2**(
                    self.RAM.conf_decoder_bits+self.RAM.col_decoder_bits))) / (2**(self.RAM.col_decoder_bits))) + self.area_dict["columndecoder"] * 2

            else:
                # In case of MTJ, banks can share sense amps so we don't have mutlitplication by two
                self.area_dict["samp_total"] = self.area_dict["mtj_subcircuits_sa"] * \
                    2**(self.RAM.conf_decoder_bits) * self.number_of_banks
                # Write driver can't be shared:
                self.area_dict["writedriver_total"] = self.area_dict["mtj_subcircuits_writedriver"] * \
                    2 * 2**(self.RAM.conf_decoder_bits) * self.number_of_banks
                self.area_dict["cs_total"] = self.area_dict["mtj_subcircuits_cs"] * 2 * 2**(
                    self.RAM.conf_decoder_bits + self.RAM.col_decoder_bits) * self.number_of_banks
                if self.RAM.row_decoder_bits == 9:
                    self.area_dict["cs_total"] = 2 * self.area_dict["cs_total"]

                self.area_dict["columndecoder_total"] = self.area_dict["columndecoder"] * 2
                RAM_area += self.area_dict["samp_total"] + \
                    self.area_dict["writedriver_total"] + \
                    self.area_dict["cs_total"]

            self.area_dict["columndecoder_sum"] = self.area_dict["columndecoder_total"] * \
            ...
            self.width_dict["configurabledecoder_wodriver"] = math.sqrt(
                self.area_dict["configurabledecoder_wodriver"])
            ...
            RAM_wordlinedriver_area = self.area_dict[self.RAM.wordlinedriver.name] * \
                self.number_of_banks
            ...
            self.width_dict["wordline_total"] = math.sqrt(
                RAM_wordlinedriver_area)
            self.area_dict["configurabledecoder"] = RAM_configurabledecoder_area
            self.width_dict["configurabledecoder"] = math.sqrt(
                RAM_configurabledecoder_area)
    ...
    def compute_distance(self):
        ...
        self.stripe_order = ["sb_sram", "sb", "sb", "cb", "cb_sram", "ic_sram", "ic", "lut_sram", "lut",
                             "cc", "ffble", "lut", "lut_sram", "ic", "ic_sram", "cb_sram", "cb", "sb", "sb", "sb_sram"]
        ...
        self.w_cb = (self.cb_mux.num_per_tile *
                     self.area_dict[self.cb_mux.name])/(self.num_cb_stripes * self.lb_height)
        self.w_sb = (self.sb_mux.num_per_tile *
                     self.area_dict[self.sb_mux.name])/(self.num_sb_stripes * self.lb_height)
        self.w_ic = (self.logic_cluster.local_mux.num_per_tile *
                     self.area_dict[self.logic_cluster.local_mux.name])/(self.num_ic_stripes * self.lb_height)
        self.w_lut = (self.specs.N*self.area_dict["lut_and_drivers"] - self.specs.N*(
            2**self.specs.K)*self.area_dict["sram"])/(self.num_lut_stripes * self.lb_height)
        # if self.specs.enable_carry_chain == 1:
        self.w_cc = self.area_dict["cc_area_total"] / \
            (self.num_cc_stripes * self.lb_height)
        self.w_ffble = self.area_dict["ffableout_area_total"] / \
            (self.num_ffble_stripes * self.lb_height)
        self.w_scb = (self.cb_mux.num_per_tile*self.area_dict[self.cb_mux.name + "_sram"] -
                      self.cb_mux.num_per_tile*self.area_dict[self.cb_mux.name])/(self.num_cbs_stripes * self.lb_height)
        self.w_ssb = (self.sb_mux.num_per_tile*self.area_dict[self.sb_mux.name + "_sram"] -
                      self.sb_mux.num_per_tile*self.area_dict[self.sb_mux.name])/(self.num_sbs_stripes * self.lb_height)
        self.w_sic = (self.logic_cluster.local_mux.num_per_tile * (
            self.area_dict[self.logic_cluster.local_mux.name + "_sram"] - self.area_dict[self.logic_cluster.local_mux.name]))/(self.num_ics_stripes * self.lb_height)
        self.w_slut = (self.specs.N*(2**self.specs.K) *
                       self.area_dict["sram"]) / (self.num_luts_stripes * self.lb_height)
                    ...
                    if (item1 == "cb" and item2 == "ic") or (item1 == "ic" and item2 == "cb"):
                        if index1 < index2:
                            distance_temp = self.dict_real_widths[self.stripe_order[index1]
                                                                  ]/self.span_stripe_fraction
                            for i in range(index1 + 1, index2):
                                distance_temp = distance_temp + \
                                    self.dict_real_widths[self.stripe_order[i]
                                                          ]/self.span_stripe_fraction
                            distance_temp = distance_temp + \
                                self.dict_real_widths[self.stripe_order[index2]
                                                      ]/self.span_stripe_fraction
                        else:
                            ...
                            distance_temp = distance_temp + \
                                self.dict_real_widths[self.stripe_order[index1]
                                                      ]/self.span_stripe_fraction
    ...
    def determine_height(self):
        ...
        while is_done == False and current_iteration < max_iteration:
            ...
            old_cost = tran_sizing.cost_function(tran_sizing.get_eval_area(
                self, "global", self.sb_mux, 0, 0), tran_sizing.get_current_delay(self, 0), self.area_opt_weight, self.delay_opt_weight)
            for i in range(-10, 11):
                ...
                self.update_wire_rc()
                self.update_delays(self.spice_interface)
                new_cost = tran_sizing.cost_function(tran_sizing.get_eval_area(
                    self, "global", self.sb_mux, 0, 0), tran_sizing.get_current_delay(self, 0), self.area_opt_weight, self.delay_opt_weight)
                if new_cost < old_cost:
                    old_cost = new_cost
                    current_best_index = i
            self.lb_height = (0.01 * (current_best_index)) * \
    ...
    def update_wires(self):
        ...
        if self.lb_height == 0:
            self.cluster_output_load.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers, 0.0, 0.0)
            self.sb_mux.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers, 1.0)
            self.cb_mux.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers, 1.0)
            self.logic_cluster.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers, 1.0, 1.0, 0.0, 0.0)
            self.routing_wire_load.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers, 0.0, 2.0, 2.0)
        else:
            sb_ratio = (self.lb_height/(self.sb_mux.num_per_tile /
                        self.num_sb_stripes)) / self.dict_real_widths["sb"]
            if sb_ratio < 1.0:
                sb_ratio = 1/sb_ratio
            ...
            cb_ratio = (self.lb_height/(self.cb_mux.num_per_tile /
                        self.num_cb_stripes)) / self.dict_real_widths["cb"]
            if cb_ratio < 1.0:
                cb_ratio = 1/cb_ratio

                # if the ratio is larger than 2.0, we can look at this stripe as two stripes put next to each other and partly fix the ratio:

            ic_ratio = (self.lb_height/(self.logic_cluster.local_mux.num_per_tile /
                        self.num_ic_stripes)) / self.dict_real_widths["ic"]
            if ic_ratio < 1.0:
                ic_ratio = 1/ic_ratio

                # if the ratio is larger than 2.0, we can look at this stripe as two stripes put next to each other and partly fix the ratio:

            lut_ratio = (self.lb_height/(self.specs.N /
                         self.num_lut_stripes)) / self.dict_real_widths["lut"]
            if lut_ratio < 1.0:
            ...
            self.cluster_output_load.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers, self.d_ffble_to_sb, self.lb_height)
            self.sb_mux.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers, sb_ratio)
            self.cb_mux.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers, cb_ratio)
            self.logic_cluster.update_wires(self.width_dict, self.wire_lengths, self.wire_layers,
                                            ic_ratio, lut_ratio, self.d_ffble_to_ic, self.d_cb_to_ic + self.lb_height)
            self.routing_wire_load.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers, self.lb_height, self.num_sb_stripes, self.num_cb_stripes)

        if self.specs.enable_carry_chain == 1:
            self.carrychain.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers)
            self.carrychainperf.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers)
            self.carrychainmux.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers)
            self.carrychaininter.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers)
            if self.specs.carry_chain_type == "skip":
                self.carrychainand.update_wires(
                    self.width_dict, self.wire_lengths, self.wire_layers)
                self.carrychainskipmux.update_wires(
                    self.width_dict, self.wire_lengths, self.wire_layers)
        if self.specs.enable_bram_block == 1:
            self.RAM.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers)

        for hardblock in self.hardblocklist:
            hardblock.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers)
            hardblock.mux.update_wires(
                self.width_dict, self.wire_lengths, self.wire_layers)
    ...
    def update_delays(self, spice_interface):
        ...
        for tran_name, tran_size in self.transistor_sizes.items():
            if not self.specs.use_finfet:
                parameter_dict[tran_name] = [
                    1e-9*tran_size*self.specs.min_tran_width]
            else:
        ...
        spice_meas = spice_interface.run(
            self.sb_mux.top_spice_path, parameter_dict)
        ...
        spice_meas = spice_interface.run(
            self.cb_mux.top_spice_path, parameter_dict)
        if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
            valid_delay = False
            tfall = 1
            trise = 1
        ...
        self.logic_cluster.local_mux.power = float(
            spice_meas["meas_avg_power"][0])
        ...
        self.logic_cluster.ble.local_output.power = float(
            spice_meas["meas_avg_power"][0])

        # General BLE output
        print("  Updating delay for " +
              self.logic_cluster.ble.general_output.name)
        ...
        self.logic_cluster.ble.general_output.power = float(
            spice_meas["meas_avg_power"][0])
        ...
        if self.specs.use_fluts:
            ...
            self.logic_cluster.ble.fmux.power = float(
                spice_meas["meas_avg_power"][0])
        ...
        for lut_input_name, lut_input in self.logic_cluster.ble.lut.input_drivers.items():
            ...
            driver_and_lut_sp_path = driver.top_spice_path.replace(
                ".sp", "_with_lut.sp")
            ...
            if (lut_input_name == "f" and self.specs.use_fluts and self.specs.K == 6) or (lut_input_name == "e" and self.specs.use_fluts and self.specs.K == 5):
                ...
                spice_meas = spice_interface.run(
                    driver_and_lut_sp_path, parameter_dict)
                if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                    valid_delay = False
                    tfall = 1
                    trise = 1
            ...
            if lut_input.delay < 0:
                print("*** Lut input delay is negative : " + str(lut_input.delay) +
                      "in path: " + driver_and_lut_sp_path + "***")
                exit(2)
            ...
            spice_meas = spice_interface.run(
                driver.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
            ...
            if driver.delay < 0:
                print("*** Lut driver delay is negative : " +
                      str(lut_input.delay) + " ***")
                exit(2)
            ...
            spice_meas = spice_interface.run(
                not_driver.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
            ...
            if not_driver.delay < 0:
                print("*** Lut not driver delay is negative : " +
                      str(lut_input.delay) + " ***")
                exit(2)
        ...
        if self.specs.enable_carry_chain == 1:
            ...
            if self.specs.carry_chain_type == "skip":
                ...
                self.carrychainand.power = float(
                    spice_meas["meas_avg_power"][0])
                ...
                self.carrychainskipmux.power = float(
                    spice_meas["meas_avg_power"][0])
        ...
        for hardblock in self.hardblocklist:
            ...
            if hardblock.parameters['num_dedicated_outputs'] > 0:
                ...
                hardblock.dedicated.power = float(
                    spice_meas["meas_avg_power"][0])
        ...
        spice_meas = spice_interface.run(
            self.RAM.rowdecoder_stage0.top_spice_path, parameter_dict)
        if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
            valid_delay = False
            tfall = 1
            trise = 1
        ...
        self.RAM.rowdecoder_stage0.power = float(
            spice_meas["meas_avg_power"][0])

        if self.RAM.valid_row_dec_size2 == 1:
            print("  Updating delay for " +
                  self.RAM.rowdecoder_stage1_size2.name)
            spice_meas = spice_interface.run(
                self.RAM.rowdecoder_stage1_size2.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
            ...
            self.RAM.rowdecoder_stage1_size2.power = float(
                spice_meas["meas_avg_power"][0])

        if self.RAM.valid_row_dec_size3 == 1:
            print("  Updating delay for " +
                  self.RAM.rowdecoder_stage1_size3.name)
            spice_meas = spice_interface.run(
                self.RAM.rowdecoder_stage1_size3.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
            ...
            self.RAM.rowdecoder_stage1_size3.power = float(
                spice_meas["meas_avg_power"][0])

        print("  Updating delay for " + self.RAM.rowdecoder_stage3.name)
        spice_meas = spice_interface.run(
            self.RAM.rowdecoder_stage3.top_spice_path, parameter_dict)
        if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
            valid_delay = False
            tfall = 1
            trise = 1
        ...
        self.RAM.rowdecoder_stage3.power = float(
            spice_meas["meas_avg_power"][0])

        if self.RAM.memory_technology == "SRAM":
            print("  Updating delay for " + self.RAM.precharge.name)
            spice_meas = spice_interface.run(
                self.RAM.precharge.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
            ...
            spice_meas = spice_interface.run(
                self.RAM.samp_part2.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
            ...
            spice_meas = spice_interface.run(
                self.RAM.samp.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
            ...
            spice_meas = spice_interface.run(
                self.RAM.writedriver.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
            ...
            spice_meas = spice_interface.run(
                self.RAM.bldischarging.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
            ...
            self.RAM.bldischarging.power = float(
                spice_meas["meas_avg_power"][0])

            print("  Updating delay for " + self.RAM.blcharging.name)
            spice_meas = spice_interface.run(
                self.RAM.blcharging.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
            ...
            self.RAM.target_bl = 0.99 * \
                float(spice_meas["meas_outputtarget"][0])
            ...
            spice_meas = spice_interface.run(
                self.RAM.mtjsamp.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
        ...
        spice_meas = spice_interface.run(
            self.RAM.columndecoder.top_spice_path, parameter_dict)
        if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
            valid_delay = False
            tfall = 1
            trise = 1
        ...
        spice_meas = spice_interface.run(
            self.RAM.configurabledecoderi.top_spice_path, parameter_dict)
        if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
            valid_delay = False
            tfall = 1
            trise = 1
        ...
        self.RAM.configurabledecoderi.power = float(
            spice_meas["meas_avg_power"][0])

        if self.RAM.cvalidobj1 == 1:
            print("  Updating delay for " +
                  self.RAM.configurabledecoder3ii.name)
            spice_meas = spice_interface.run(
                self.RAM.configurabledecoder3ii.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
            ...
            self.RAM.configurabledecoder3ii.power = float(
                spice_meas["meas_avg_power"][0])

        if self.RAM.cvalidobj2 == 1:
            print("  Updating delay for " +
                  self.RAM.configurabledecoder2ii.name)
            spice_meas = spice_interface.run(
                self.RAM.configurabledecoder2ii.top_spice_path, parameter_dict)
            if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
                valid_delay = False
                tfall = 1
                trise = 1
            ...
            self.RAM.configurabledecoder2ii.power = float(
                spice_meas["meas_avg_power"][0])

        print("  Updating delay for " + self.RAM.configurabledecoderiii.name)
        spice_meas = spice_interface.run(
            self.RAM.configurabledecoderiii.top_spice_path, parameter_dict)
        if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
            valid_delay = False
            tfall = 1
            trise = 1
        ...
        self.RAM.configurabledecoderiii.power = float(
            spice_meas["meas_avg_power"][0])

        print("  Updating delay for " + self.RAM.pgateoutputcrossbar.name)
        spice_meas = spice_interface.run(
            self.RAM.pgateoutputcrossbar.top_spice_path, parameter_dict)
        if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
            valid_delay = False
            tfall = 1
            trise = 1
        ...
        self.RAM.pgateoutputcrossbar.power = float(
            spice_meas["meas_avg_power"][0])
        self.delay_dict["rep_crit_path"] = crit_path_delay

        print("  Updating delay for " + self.RAM.wordlinedriver.name)
        spice_meas = spice_interface.run(
            self.RAM.wordlinedriver.top_spice_path, parameter_dict)
        if spice_meas["meas_total_tfall"][0] == "failed" or spice_meas["meas_total_trise"][0] == "failed":
            valid_delay = False
            tfall = 1
            trise = 1
    ...
    def update_power(self, spice_interface):
        ...
        if self.RAM.valid_row_dec_size2 == 1:
            ram_decoder_stage1_delay = max(
                ram_decoder_stage1_delay, self.RAM.rowdecoder_stage1_size2.delay)
        if self.RAM.valid_row_dec_size3 == 1:
            ram_decoder_stage1_delay = max(
                self.RAM.rowdecoder_stage1_size3.delay, ram_decoder_stage1_delay)
        ...
        if self.RAM.cvalidobj1 == 1:
            configurable_decoder_delay = max(
                self.RAM.configurabledecoder3ii.delay, configurable_decoder_delay)
        if self.RAM.cvalidobj2 == 1:
            configurable_decoder_delay = max(
                self.RAM.configurabledecoder2ii.delay, configurable_decoder_delay)
        ...
        if self.RAM.memory_technology == "SRAM":
            self.RAM.T1 = max(self.RAM.estimated_rowdecoder_delay,
                              configurable_decoder_delay, self.RAM.precharge.delay)
            self.RAM.T2 = self.RAM.wordlinedriver.delay + \
                self.RAM.samp.delay + self.RAM.samp_part2.delay
            self.RAM.frequency = max(
                self.RAM.T1 + self.RAM.T2, configurable_decoder_delay + configurable_decoder_drive)
        ...
        # 3. The Tmicro_reg_setup value is hardcoded as 3e-9
            ...
            self.RAM.T1 = max(self.RAM.estimated_rowdecoder_delay,
                              configurable_decoder_delay, self.RAM.bldischarging.delay)
            self.RAM.T2 = self.RAM.T1 + \
                max(self.RAM.wordlinedriver.delay,
                    configurable_decoder_drive) + self.RAM.blcharging.delay
        ...
        if self.RAM.memory_technology == "SRAM":
            ...
            spice_meas = spice_interface.run(
                self.RAM.power_sram_read.top_spice_path, parameter_dict)
            self.RAM.power_sram_read.power_selected = float(
                spice_meas["meas_avg_power_selected"][0])
            self.RAM.power_sram_read.power_unselected = float(
                spice_meas["meas_avg_power_unselected"][0])

            spice_meas = spice_interface.run(
                self.RAM.power_sram_writelh.top_spice_path, parameter_dict)
            self.RAM.power_sram_writelh.power_selected_writelh = float(
                spice_meas["meas_avg_power_selected"][0])

            spice_meas = spice_interface.run(
                self.RAM.power_sram_writehh.top_spice_path, parameter_dict)
            self.RAM.power_sram_writehh.power_selected_writehh = float(
                spice_meas["meas_avg_power_selected"][0])

            spice_meas = spice_interface.run(
                self.RAM.power_sram_writep.top_spice_path, parameter_dict)
            self.RAM.power_sram_writep.power_selected_writep = float(
                spice_meas["meas_avg_power_selected"][0])
            ...
            print("Energy Writep " +
                  str(self.RAM.power_sram_writep.power_selected_writep * self.RAM.frequency))

            read_energy = (self.RAM.power_sram_read.power_selected +
                           self.RAM.power_sram_read.power_unselected) * self.RAM.frequency
            write_energy = ((self.RAM.power_sram_writelh.power_selected_writelh + self.RAM.power_sram_writehh.power_selected_writehh) /
                            2 + self.RAM.power_sram_read.power_unselected) * self.RAM.frequency

            self.RAM.core_energy = (self.RAM.read_to_write_ratio * read_energy +
                                    write_energy) / (1 + self.RAM.read_to_write_ratio)

        else:
            print("Measuring MTJ power ")
            spice_meas = spice_interface.run(
                self.RAM.power_mtj_write.top_spice_path, parameter_dict)
            self.RAM.power_mtj_write.powerpl = float(
                spice_meas["meas_avg_power_selected"][0])
            self.RAM.power_mtj_write.powernl = float(
                spice_meas["meas_avg_power_selectedn"][0])
            self.RAM.power_mtj_write.powerph = float(
                spice_meas["meas_avg_power_selectedh"][0])
            self.RAM.power_mtj_write.powernh = float(
                spice_meas["meas_avg_power_selectedhn"][0])
            ...
            spice_meas = spice_interface.run(
                self.RAM.power_mtj_read.top_spice_path, parameter_dict)
            self.RAM.power_mtj_read.powerl = float(
                spice_meas["meas_avg_power_readl"][0])
            self.RAM.power_mtj_read.powerh = float(
                spice_meas["meas_avg_power_readh"][0])
            ...
            read_energy = ((self.RAM.power_mtj_read.powerl +
                           self.RAM.power_mtj_read.powerh) * self.RAM.frequency)
            write_energy = ((self.RAM.power_mtj_write.powerph - self.RAM.power_mtj_write.powernh +
                            self.RAM.power_mtj_write.powerpl - self.RAM.power_mtj_write.powernl) * self.RAM.frequency)/4
            self.RAM.core_energy = (self.RAM.read_to_write_ratio * read_energy +
                                    write_energy) / (1 + self.RAM.read_to_write_ratio)
        ...
        peripheral_energy = self.RAM.row_decoder_bits / 2 * \
            self.RAM.rowdecoder_stage0.power * self.RAM.number_of_banks
        if self.RAM.valid_row_dec_size2 == 1 and self.RAM.valid_row_dec_size3 == 1:
            peripheral_energy += (self.RAM.rowdecoder_stage1_size3.power +
                                  self.RAM.rowdecoder_stage1_size2.power)/2
        ...
        peripheral_energy += self.RAM.configurabledecoderi.power * \
            self.RAM.conf_decoder_bits / 2 * self.RAM.number_of_banks
        peripheral_energy += self.RAM.configurabledecoderiii.power * \
            (1 + 2**self.RAM.conf_decoder_bits)/2

        # Convert to energy
        peripheral_energy = peripheral_energy * self.RAM.frequency

        # Add read-specific components
        self.RAM.peripheral_energy_read = peripheral_energy + self.RAM.pgateoutputcrossbar.power * \
            (1 + 2**self.RAM.conf_decoder_bits)//2 * self.RAM.frequency
        ...
        self.RAM.peripheral_energy_write = peripheral_energy + \
            (2 ** self.RAM.conf_decoder_bits *
             self.RAM.configurabledecoderiii.power / 2) * self.RAM.frequency
        # Add write-specific components (Write enable wires)
        self.RAM.peripheral_energy_write += ((1 + 2 ** self.RAM.conf_decoder_bits)
                                             * self.RAM.configurabledecoderiii.power) * self.RAM.frequency
        # We want energy per bit per OP:
        self.RAM.peripheral_energy_write //= 2 ** self.RAM.conf_decoder_bits

        print("Core read and write energy: " +
              str(read_energy) + " and " + str(write_energy))
        print("Core energy per bit: " + str(self.RAM.core_energy))
        print("Peripheral energy per bit: " + str((self.RAM.peripheral_energy_read *
              self.RAM.read_to_write_ratio + self.RAM.peripheral_energy_write) / (1 + self.RAM.read_to_write_ratio)))
    ...
    def print_specs(self):
        ...
        print("  Number of BLE outputs to general routing: " +
              str(self.specs.num_ble_general_outputs))
        print("  Number of BLE outputs to local routing: " +
              str(self.specs.num_ble_local_outputs))
        print("  Number of cluster outputs: " +
              str(self.specs.num_cluster_outputs))
    ...
    def print_details(self, report_file):
        ...
        utils.print_and_write(
            report_file, "|------------------------------------------------------------------------------|")
        utils.print_and_write(
            report_file, "|   FPGA Implementation Details                                                |")
        utils.print_and_write(
            report_file, "|------------------------------------------------------------------------------|")
        utils.print_and_write(report_file, "")
    ...
    def _generate_basic_subcircuits(self):
        ...
        basic_subcircuits.ptran_generate(
            self.basic_subcircuits_filename, self.specs.use_finfet)
        basic_subcircuits.ptran_pmos_generate(
            self.basic_subcircuits_filename, self.specs.use_finfet)
        # Generate transmission gate subcircuit
        basic_subcircuits.tgate_generate(
            self.basic_subcircuits_filename, self.specs.use_finfet)
        basic_subcircuits.tgate_generate_lp(
            self.basic_subcircuits_filename, self.specs.use_finfet)
        # Generate level-restore subcircuit
        basic_subcircuits.rest_generate(
            self.basic_subcircuits_filename, self.specs.use_finfet)
        # Generate inverter subcircuit
        basic_subcircuits.inverter_generate(
            self.basic_subcircuits_filename, self.specs.use_finfet, self.specs.memory_technology)
        # Generate nand2
        basic_subcircuits.nand2_generate(
            self.basic_subcircuits_filename, self.specs.use_finfet)
        basic_subcircuits.nand2_generate_lp(
            self.basic_subcircuits_filename, self.specs.use_finfet)
        # Generate nand3
        basic_subcircuits.nand3_generate(
            self.basic_subcircuits_filename, self.specs.use_finfet)
        basic_subcircuits.nand3_generate_lp(
            self.basic_subcircuits_filename, self.specs.use_finfet)
        # generate ram tgate
        basic_subcircuits.RAM_tgate_generate(
            self.basic_subcircuits_filename, self.specs.use_finfet)
        basic_subcircuits.RAM_tgate_generate_lp(
            self.basic_subcircuits_filename, self.specs.use_finfet)
    ...
    def _generate_process_data(self):
        ...
        process_data_file.write(".PARAM supply_v = " +
                                str(self.specs.vdd) + "\n")
        process_data_file.write(".PARAM sram_v = " +
                                str(self.specs.vsram) + "\n")
        process_data_file.write(".PARAM sram_n_v = " +
                                str(self.specs.vsram_n) + "\n")
        process_data_file.write(".PARAM Rcurrent = " +
                                str(self.specs.worst_read_current) + "\n")
        process_data_file.write(
            ".PARAM supply_v_lp = " + str(self.specs.vdd_low_power) + "\n\n")
        ...
        if use_lp_transistor == 0:
            process_data_file.write(
                ".PARAM sense_v = " + str(self.specs.vdd - self.specs.sense_dv) + "\n\n")
        else:
            process_data_file.write(
                ".PARAM sense_v = " + str(self.specs.vdd_low_power - self.specs.sense_dv) + "\n\n")

        process_data_file.write(
            ".PARAM mtj_worst_high = " + str(self.specs.MTJ_Rhigh_worstcase) + "\n")
        process_data_file.write(
            ".PARAM mtj_worst_low = " + str(self.specs.MTJ_Rlow_worstcase) + "\n")
        process_data_file.write(
            ".PARAM mtj_nominal_low = " + str(self.specs.MTJ_Rlow_nominal) + "\n\n")
        process_data_file.write(
            ".PARAM mtj_nominal_high = " + str(6250) + "\n\n")
        process_data_file.write(".PARAM vref = " + str(self.specs.vref) + "\n")
        process_data_file.write(
            ".PARAM vclmp = " + str(self.specs.vclmp) + "\n")

        process_data_file.write("* Geometry\n")
        process_data_file.write(
            ".PARAM gate_length = " + str(self.specs.gate_length) + "n\n")
        process_data_file.write(
            ".PARAM trans_diffusion_length = " + str(self.specs.trans_diffusion_length) + "n\n")
        process_data_file.write(
            ".PARAM min_tran_width = " + str(self.specs.min_tran_width) + "n\n")
        process_data_file.write(
            ".param rest_length_factor=" + str(self.specs.rest_length_factor) + "\n")
        process_data_file.write("\n")
        ...
        process_data_file.write("* Device models\n")
        process_data_file.write(
            ".LIB \"" + self.specs.model_path + "\" " + self.specs.model_library + "\n\n")
        process_data_file.write(".ENDL PROCESS_DATA")
        process_data_file.close()
            ...
            # If it's an inverter, we have to add both NMOS and PMOS sizes
                ...
                if inv_ratios == None:
                    ...
                    if inv_ratios[element_name] < 1:
                        ...
                        if not use_finfet:
                            new_sizes[element_name + "_nmos"] = combo[i] / \
                                inv_ratios[element_name]
                        else:
                            new_sizes[element_name +
                                      "_nmos"] = round(combo[i]/inv_ratios[element_name])
                            # new_sizes[element_name + "_nmos"] = combo[i]
                        new_sizes[element_name + "_pmos"] = combo[i]
                    else:
                        ...
                        if not use_finfet:
                            ...
                            new_sizes[element_name +
                                      "_pmos"] = round(combo[i]*inv_ratios[element_name])
                            # new_sizes[element_name + "_pmos"] = combo[i]
        ...
        for tran in self.transistor_area_list:
            ...
            if "inv_" in tran[0] or "tgate_" in tran[0]:
                ...
                if comp_name in comp_dict:
                    ...
                    comp_area = comp_dict[comp_name]["nmos"] + \
                        comp_dict[comp_name]["pmos"]
