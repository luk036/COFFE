...
def get_eval_delay(fpga_inst, opt_type, subcircuit, tfall, trise, low_voltage, is_ram_component, is_cc_component):
    ...
    if is_cc_component:
        ...
        if fpga_inst.specs.carry_chain_type == "ripple":
            path_delay = (fpga_inst.specs.N * fpga_inst.specs.FAs_per_flut - 2) * \
                fpga_inst.carrychain.delay + fpga_inst.carrychainperf.delay + \
                fpga_inst.carrychaininter.delay
            ...
            path_delay = (fpga_inst.carrychain.delay * skip_size + fpga_inst.carrychainand.delay + fpga_inst.carrychainskipmux.delay) + 2 * fpga_inst.carrychainskipmux.delay + \
                fpga_inst.carrychain.delay * skip_size + fpga_inst.carrychainperf.delay + \
                (3 - fpga_inst.specs.FAs_per_flut) * \
                fpga_inst.carrychaininter.delay
        return path_delay
    ...
    if opt_type == "local":
        ...
        # Local mux
        path_delay += fpga_inst.logic_cluster.local_mux.delay * \
            fpga_inst.logic_cluster.local_mux.delay_weight
        # LUT
        ...
        # Local BLE output
        path_delay += fpga_inst.logic_cluster.ble.local_output.delay * \
            fpga_inst.logic_cluster.ble.local_output.delay_weight
        # General BLE output
        path_delay += fpga_inst.logic_cluster.ble.general_output.delay * \
            fpga_inst.logic_cluster.ble.general_output.delay_weight
        if fpga_inst.specs.use_fluts:
            path_delay += fpga_inst.logic_cluster.ble.fmux.delay * \
                fpga_inst.logic_cluster.ble.lut.delay_weight

        if fpga_inst.specs.enable_carry_chain == 1:
            path_delay += fpga_inst.carrychainmux.delay * \
                fpga_inst.logic_cluster.ble.lut.delay_weight
        ...
        if fpga_inst.RAM.memory_technology == "SRAM":
            ram_delay += fpga_inst.RAM.writedriver.delay + fpga_inst.RAM.samp.delay + \
                fpga_inst.RAM.samp_part2.delay + fpga_inst.RAM.precharge.delay
        else:
            ram_delay += fpga_inst.RAM.bldischarging.delay + \
                fpga_inst.RAM.blcharging.delay + fpga_inst.RAM.mtjsamp.delay
...
def get_final_delay(fpga_inst, opt_type, subcircuit, tfall, trise, is_ram_component, is_cc_component):
    ...
    if delay < 0:
        ...
        print("***Negative delay: " + str(delay) +
              " in " + subcircuit.name + " ***")
        exit(2)
    subcircuit.delay = delay
    subcircuit.trise = trise
    ...
    if opt_type == "local":
        ...
        # final delay should not be negative, something went wrong :(
        if path_delay < 0:
            print("***Negative path delay: " + str(path_delay) +
                  " in " + subcircuit.name + " ***")
            exit(2)
...
def erf_inverter(sp_path,
    ...
    if not fpga_inst.specs.use_finfet:
        fpga_inst.transistor_sizes[nmos_name] = (
            nmos_size/fpga_inst.specs.min_tran_width)
        fpga_inst.transistor_sizes[pmos_name] = (
            pmos_size/fpga_inst.specs.min_tran_width)
    else:
...
def erf(sp_path,
    ...
    if not fpga_inst.specs.use_finfet:
        for tran_name, tran_size in fpga_inst.transistor_sizes.items():
            parameter_dict[tran_name] = [
                1e-9*tran_size*fpga_inst.specs.min_tran_width]
    ...
    while not erf_tolerance_met:
        ...
        for circuit_element in element_names:
            ...
            if not fpga_inst.specs.use_finfet:
                nmos_nm_size = int(
                    parameter_dict[circuit_element + "_nmos"][0]/1e-9)
                pmos_nm_size = int(
                    parameter_dict[circuit_element + "_pmos"][0]/1e-9)
            else:
                nmos_nm_size = int(
                    parameter_dict[circuit_element + "_nmos"][0])
                pmos_nm_size = int(
                    parameter_dict[circuit_element + "_pmos"][0])
...
def erf_combo(fpga_inst,
    ...
    fpga_inst._update_transistor_sizes(
        element_names, combo, fpga_inst.specs.use_finfet)
...
def run_combo(fpga_inst, sp_path, element_names, combo, erf_ratios, spice_interface):
    ...
    # Update transistor sizes
    fpga_inst._update_transistor_sizes(
        element_names, combo, fpga_inst.specs.use_finfet, erf_ratios)
    # Calculate area of everything
    fpga_inst.update_area()
    # Re-calculate wire lengths
    fpga_inst.update_wires()
...
def search_ranges(sizing_ranges, fpga_inst, sizable_circuit, opt_type, re_erf, area_opt_weight,
    ...
    for combo in sizing_combos:
        ...
        fpga_inst.update_area()
        # Get evaluation area
        area_list.append(get_eval_area(fpga_inst, opt_type,
                         sizable_circuit, is_ram_component, is_cc_component))
        # Re-calculate wire lengths
        fpga_inst.update_wires()
        # Update wire resistance and capacitance
        fpga_inst.update_wire_rc()
    ...
    if not fpga_inst.specs.use_finfet:
        for tran_name, tran_size in fpga_inst.transistor_sizes.items():
            current_tran_sizes[tran_name] = 1e-9 * \
                tran_size*fpga_inst.specs.min_tran_width
    else:
        for tran_name, tran_size in fpga_inst.transistor_sizes.items():
    ...
    spice_meas = spice_interface.run(
        sizable_circuit.top_spice_path, parameter_dict)
    ...
    for i in range(len(sizing_combos)):
        ...
        if spice_meas["meas_logic_low_voltage"][i] == "failed":
            ...
            meas_logic_low_voltage.append(
                float(spice_meas["meas_logic_low_voltage"][i]))
    ...
    for i in range(len(tfall_trise_list)):
        ...
        delay = get_eval_delay(fpga_inst, opt_type, sizable_circuit,
                               tfall_trise[0], tfall_trise[1], meas_logic_low_voltage[i], is_ram_component, is_cc_component)
    ...
    for i in range(re_erf_num):
        ...
        delay = get_final_delay(fpga_inst, opt_type, sizable_circuit,
                                tfall, trise, is_ram_component, is_cc_component)
        # Get area (run_combo will have updated the area numbers for us so we can get it
        # directly)
        area = get_final_area(
            fpga_inst, opt_type, sizable_circuit, is_ram_component, is_cc_component)
        # Calculate cost
        cost = cost_function(area, delay, area_opt_weight, delay_opt_weight)
        # Add to best results (cost, combo_index, area, delay, tfall, trise, erf_ratios)
        best_results.append(
            (cost, cost_list[i][1], area, delay, tfall, trise, erf_ratios))
    ...
    # Update transistor sizes
    fpga_inst._update_transistor_sizes(
        element_names, best_combo, fpga_inst.specs.use_finfet, best_combo_erf_ratios)
    # Calculate area of everything
    fpga_inst.update_area()
    # Re-calculate wire lengths
    fpga_inst.update_wires()
...
def _print_sizing_ranges(subcircuit_name, sizing_ranges_list):
    ...
    for sizing_ranges in sizing_ranges_list:
        for name, size_range in sizing_ranges.items():
            sizing_strings[name] = sizing_strings[name] + \
                ("[" + str(size_range[0]) + " -> " +
                 str(size_range[1]) + "]").ljust(11)
...
def size_subcircuit_transistors(fpga_inst,
    ...
    print("|------------------------------------------------------------------------------|")
    print("|    Transistor sizing on subcircuit: " +
          subcircuit.name.ljust(41) + "|")
    print("|------------------------------------------------------------------------------|")
    print("")
    ...
    tran_names = format_transistor_names_to_basic_subcircuits(
        subcircuit.transistor_names)
    ...
    if len(sizing_ranges_set_list) > 1:
        ...
        # Find the combo that is near the middle of all ranges
        middle_combo = get_middle_value_config(
            element_names, sizing_ranges_complete)
    ...
    for set_num in range(len(sizing_ranges_set_list)):
        ...
        while not sizes_are_valid:
            ...
            sizes_are_valid = update_sizing_ranges(
                sizing_ranges, sizing_results_set)
...
def print_results(opt_type, sizing_results_list, area_results_list, delay_results_list):
    ...
    for subcircuit_name in subcircuit_names:
        ...
        if opt_type == "local":
            ...
            for i in range(len(area_results_list)):
                ...
                area_delay_strings.append(str(round(subcircuit_area, 3)) + "\t" + str(
                    round(subcircuit_delay, 14)) + "\t" + str(round(subcircuit_cost, 5)))
    ...
    for i in range(len(area_results_list)):
        ...
        totals_strings.append(str(round(total_area/1000000, 3)) + "\t" +
                              str(round(total_delay, 14)) + "\t" + str(round(total_cost, 5)))
...
def export_sizing_results(results_filename, sizing_results_list, area_results_list, delay_results_list):
    ...
    for subcircuit_name in subcircuit_names:
        ...
        for name, size_string in sizing_strings.items():
            results_file.write("".join(name.ljust(col_width)
                                       ) + ": " + size_string + "\n")
        results_file.write("\n")

        results_file.write(("Area").ljust(
            20) + ("Delay").ljust(20) + ("APD").ljust(20) + "\n")
        area_delay_strings = []
        for i in range(len(area_results_list)):
            area_results = area_results_list[i]
            delay_results = delay_results_list[i]
            ...
            subcircuit_delay = delay_results[subcircuit_name]
            subcircuit_cost = subcircuit_area*subcircuit_delay
            area_delay_strings.append((str(subcircuit_area)).ljust(
                20) + (str(subcircuit_delay)).ljust(20) + (str(subcircuit_cost)).ljust(20))
    ...
    for i in range(len(area_results_list)):
        ...
        total_delay = delay_results["rep_crit_path"]
        total_cost = total_area*total_delay
        totals_strings.append((str(total_area)).ljust(
            20) + (str(total_delay)).ljust(20) + (str(total_cost)).ljust(20))
    for total_str in totals_strings:
...
def check_if_done(sizing_results_list, area_results_list, delay_results_list, area_opt_weight, delay_opt_weight):
    ...
    for i in range(len(area_results_list)):
        ...
        total_cost = cost_function(
            total_area, total_delay, area_opt_weight, delay_opt_weight)
...
def size_fpga_transistors(fpga_inst, run_options, spice_interface):
    ...
    while not is_done:
        ...
        current_cost = cost_function(get_eval_area(fpga_inst, "global", fpga_inst.sb_mux, 0, 0), get_current_delay(
            fpga_inst, 0), area_opt_weight, delay_opt_weight)
        ...
        for hardblock in fpga_inst.hardblocklist:
            ...
            if hardblock.parameters['num_dedicated_outputs'] > 0:
                ...
                if iteration == 1:
                    ...
                    starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                        hardblock.dedicated.initial_transistor_sizes)
                else:
                    starting_transistor_sizes = sizing_results_list[len(
                        sizing_results_list)-1][name]
                ...
                if quick_mode_dict[name] == 1:
                    sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                        fpga_inst, hardblock.dedicated, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 0)
                else:
                    sizing_results_dict[name] = sizing_results_list[len(
                        sizing_results_list)-1][name]
                    sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
                        sizing_results_list)-1][name]
            ...
            if iteration == 1:
                quick_mode_dict[name] = 1
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    hardblock.mux.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict[name] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, hardblock.mux, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 0)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
        ...
        if quick_mode_dict[name] == 1:
            sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                fpga_inst, fpga_inst.sb_mux, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 0)
        else:
            sizing_results_dict[name] = sizing_results_list[len(
                sizing_results_list)-1][name]
            sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
        ...
        if iteration == 1:
            quick_mode_dict[name] = 1
            starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                fpga_inst.cb_mux.initial_transistor_sizes)
        else:
            starting_transistor_sizes = sizing_results_list[len(
                sizing_results_list)-1][name]
        ...
        if quick_mode_dict[name] == 1:
            sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                fpga_inst, fpga_inst.cb_mux, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 0)
        else:
            sizing_results_dict[name] = sizing_results_list[len(
                sizing_results_list)-1][name]
            sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
        ...
        if iteration == 1:
            quick_mode_dict[name] = 1
            starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                fpga_inst.logic_cluster.local_mux.initial_transistor_sizes)
        else:
            starting_transistor_sizes = sizing_results_list[len(
                sizing_results_list)-1][name]
        ...
        if quick_mode_dict[name] == 1:
            sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                fpga_inst, fpga_inst.logic_cluster.local_mux, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 0)
        else:
            sizing_results_dict[name] = sizing_results_list[len(
                sizing_results_list)-1][name]
            sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
        ...
        if iteration == 1:
            quick_mode_dict[name] = 1
            starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                fpga_inst.logic_cluster.ble.lut.initial_transistor_sizes)
        else:
            starting_transistor_sizes = sizing_results_list[len(
                sizing_results_list)-1][name]
        ...
        if quick_mode_dict[name] == 1:
            sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                fpga_inst, fpga_inst.logic_cluster.ble.lut, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 0)
        else:
            sizing_results_dict[name] = sizing_results_list[len(
                sizing_results_list)-1][name]
            sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
        ...
        if fpga_inst.specs.use_fluts:
            ...
            if iteration == 1:
                quick_mode_dict[name] = 1
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.logic_cluster.ble.fmux.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict[name] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.logic_cluster.ble.fmux, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 0)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
        ...
        for input_driver_name, input_driver in fpga_inst.logic_cluster.ble.lut.input_drivers.items():
            ...
            if iteration == 1:
                ...
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    input_driver.driver.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][input_driver.driver.name]

            # Size the transistors of this subcircuit
            if quick_mode_dict["input_drivers"] == 1:
                sizing_results_dict[input_driver.driver.name], sizing_results_detailed_dict[input_driver.driver.name] = size_subcircuit_transistors(
                    fpga_inst, input_driver.driver, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 0)
            else:
                sizing_results_dict[input_driver.driver.name] = sizing_results_list[len(
                    sizing_results_list)-1][input_driver.driver.name]
                sizing_results_detailed_dict[input_driver.driver.name] = sizing_results_detailed_list[len(
                    sizing_results_list)-1][input_driver.driver.name]
            ...
            if iteration == 1:
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    input_driver.not_driver.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][input_driver.not_driver.name]

            # Size the transistors of this subcircuit
            if quick_mode_dict["input_drivers"] == 1:
                sizing_results_dict[input_driver.not_driver.name], sizing_results_detailed_dict[input_driver.not_driver.name] = size_subcircuit_transistors(
                    fpga_inst, input_driver.not_driver, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 0)
            else:
                sizing_results_dict[input_driver.not_driver.name] = sizing_results_list[len(
                    sizing_results_list)-1][input_driver.not_driver.name]
                sizing_results_detailed_dict[input_driver.not_driver.name] = sizing_results_detailed_list[len(
                    sizing_results_list)-1][input_driver.not_driver.name]
        ...
        if iteration == 1:
            quick_mode_dict[name] = 1
            starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                fpga_inst.logic_cluster.ble.local_output.initial_transistor_sizes)
        else:
            starting_transistor_sizes = sizing_results_list[len(
                sizing_results_list)-1][name]
        ...
        if quick_mode_dict[name] == 1:
            sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                fpga_inst, fpga_inst.logic_cluster.ble.local_output, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 0)
        else:
            sizing_results_dict[name] = sizing_results_list[len(
                sizing_results_list)-1][name]
            sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
        ...
        if iteration == 1:
            quick_mode_dict[name] = 1
            starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                fpga_inst.logic_cluster.ble.general_output.initial_transistor_sizes)
        else:
            starting_transistor_sizes = sizing_results_list[len(
                sizing_results_list)-1][name]
        ...
        if quick_mode_dict[name] == 1:
            sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                fpga_inst, fpga_inst.logic_cluster.ble.general_output, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 0)
        else:
            sizing_results_dict[name] = sizing_results_list[len(
                sizing_results_list)-1][name]
            sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
        ...
        if quick_mode_dict[name] == 1:
            ...
            current_cost = cost_function(get_eval_area(fpga_inst, "global", fpga_inst.sb_mux, 1, 0), get_current_delay(
                fpga_inst, 1), area_opt_weight, delay_opt_weight)
        ...
        if fpga_inst.specs.enable_carry_chain == 1:
            ...
            if iteration == 1:
                quick_mode_dict[name] = 1
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.carrychain.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict[name] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.carrychain, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 1)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if quick_mode_dict[name] == 1:
                ...
                past_cost = current_cost
                current_cost = cost_function(get_eval_area(fpga_inst, "global", fpga_inst.sb_mux, 1, 1), get_current_delay(
                    fpga_inst, 0), area_opt_weight, delay_opt_weight)
                if (past_cost - current_cost)/past_cost < fpga_inst.specs.quick_mode_threshold:
                    quick_mode_dict[name] = 0
            ...
            if iteration == 1:
                quick_mode_dict[name] = 1
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.carrychainperf.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict[name] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.carrychainperf, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 1)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if iteration == 1:
                quick_mode_dict[name] = 1
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.carrychaininter.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict[name] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.carrychaininter, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 1)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if fpga_inst.specs.carry_chain_type == "skip":
                ...
                if iteration == 1:
                    quick_mode_dict[name] = 1
                    starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                        fpga_inst.carrychainand.initial_transistor_sizes)
                else:
                    starting_transistor_sizes = sizing_results_list[len(
                        sizing_results_list)-1][name]
                ...
                if quick_mode_dict[name] == 1:
                    sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                        fpga_inst, fpga_inst.carrychainand, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 1)
                else:
                    sizing_results_dict[name] = sizing_results_list[len(
                        sizing_results_list)-1][name]
                    sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if fpga_inst.specs.carry_chain_type == "skip":
                ...
                if iteration == 1:
                    quick_mode_dict[name] = 1
                    starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                        fpga_inst.carrychainskipmux.initial_transistor_sizes)
                else:
                    starting_transistor_sizes = sizing_results_list[len(
                        sizing_results_list)-1][name]
                ...
                if quick_mode_dict[name] == 1:
                    sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                        fpga_inst, fpga_inst.carrychainskipmux, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 1)
                else:
                    sizing_results_dict[name] = sizing_results_list[len(
                        sizing_results_list)-1][name]
                    sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if iteration == 1:
                quick_mode_dict[name] = 1
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.carrychainmux.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict[name] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.carrychainmux, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 0, 0)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
        ...
        if fpga_inst.specs.enable_bram_block == 1:
            ...
            if iteration == 1:
                quick_mode_dict[name] = 1
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.RAM.pgateoutputcrossbar.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict[name] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.RAM.pgateoutputcrossbar, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if iteration == 1:
                quick_mode_dict[name] = 1
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.RAM.wordlinedriver.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict[name] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.RAM.wordlinedriver, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if fpga_inst.RAM.memory_technology == "SRAM":
                ...
                if iteration == 1:
                    quick_mode_dict[name] = 1
                    starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                        fpga_inst.RAM.precharge.initial_transistor_sizes)
                else:
                    starting_transistor_sizes = sizing_results_list[len(
                        sizing_results_list)-1][name]
                ...
                if quick_mode_dict[name] == 1:
                    sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                        fpga_inst, fpga_inst.RAM.precharge, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
                else:
                    sizing_results_dict[name] = sizing_results_list[len(
                        sizing_results_list)-1][name]
                    sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
                ...
                if iteration == 1:
                    quick_mode_dict[name] = 1
                    starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                        fpga_inst.RAM.bldischarging.initial_transistor_sizes)
                else:
                    starting_transistor_sizes = sizing_results_list[len(
                        sizing_results_list)-1][name]
                ...
                if quick_mode_dict[name] == 1:
                    sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                        fpga_inst, fpga_inst.RAM.bldischarging, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
                else:
                    sizing_results_dict[name] = sizing_results_list[len(
                        sizing_results_list)-1][name]
                    sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if iteration == 1:
                ...
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.RAM.rowdecoder_stage3.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict["rowdecoder"] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.RAM.rowdecoder_stage3, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if fpga_inst.RAM.valid_row_dec_size3 == 1:
                ...
                if iteration == 1:
                    starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                        fpga_inst.RAM.rowdecoder_stage1_size3.initial_transistor_sizes)
                else:
                    starting_transistor_sizes = sizing_results_list[len(
                        sizing_results_list)-1][name]
                ...
                if quick_mode_dict["rowdecoder"] == 1:
                    sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                        fpga_inst, fpga_inst.RAM.rowdecoder_stage1_size3, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
                else:
                    sizing_results_dict[name] = sizing_results_list[len(
                        sizing_results_list)-1][name]
                    sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if fpga_inst.RAM.valid_row_dec_size2 == 1:
                ...
                if iteration == 1:
                    starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                        fpga_inst.RAM.rowdecoder_stage1_size2.initial_transistor_sizes)
                else:
                    starting_transistor_sizes = sizing_results_list[len(
                        sizing_results_list)-1][name]
                ...
                if quick_mode_dict["rowdecoder"] == 1:
                    sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                        fpga_inst, fpga_inst.RAM.rowdecoder_stage1_size2, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
                else:
                    sizing_results_dict[name] = sizing_results_list[len(
                        sizing_results_list)-1][name]
                    sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if iteration == 1:
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.RAM.rowdecoder_stage0.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict["rowdecoder"] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.RAM.rowdecoder_stage0, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if iteration == 1:
                quick_mode_dict[name] = 1
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.RAM.RAM_local_mux.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict[name] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.RAM.RAM_local_mux, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if iteration == 1:
                ...
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.RAM.configurabledecoderiii.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict["confdec"] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.RAM.configurabledecoderiii, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if fpga_inst.RAM.cvalidobj1 == 1:
                ...
                if iteration == 1:
                    starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                        fpga_inst.RAM.configurabledecoder3ii.initial_transistor_sizes)
                else:
                    starting_transistor_sizes = sizing_results_list[len(
                        sizing_results_list)-1][name]
                ...
                if quick_mode_dict["confdec"] == 1:
                    sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                        fpga_inst, fpga_inst.RAM.configurabledecoder3ii, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
                else:
                    sizing_results_dict[name] = sizing_results_list[len(
                        sizing_results_list)-1][name]
                    sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if fpga_inst.RAM.cvalidobj2 == 1:
                ...
                if iteration == 1:
                    starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                        fpga_inst.RAM.configurabledecoder2ii.initial_transistor_sizes)
                else:
                    starting_transistor_sizes = sizing_results_list[len(
                        sizing_results_list)-1][name]
                ...
                if quick_mode_dict["confdec"] == 1:
                    sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                        fpga_inst, fpga_inst.RAM.configurabledecoder2ii, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
                else:
                    sizing_results_dict[name] = sizing_results_list[len(
                        sizing_results_list)-1][name]
                    sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if iteration == 1:
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.RAM.configurabledecoderi.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict["confdec"] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.RAM.configurabledecoderi, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
            ...
            if iteration == 1:
                quick_mode_dict[name] = 1
                starting_transistor_sizes = format_transistor_sizes_to_basic_subciruits(
                    fpga_inst.RAM.columndecoder.initial_transistor_sizes)
            else:
                starting_transistor_sizes = sizing_results_list[len(
                    sizing_results_list)-1][name]
            ...
            if quick_mode_dict[name] == 1:
                sizing_results_dict[name], sizing_results_detailed_dict[name] = size_subcircuit_transistors(
                    fpga_inst, fpga_inst.RAM.columndecoder, opt_type, re_erf, area_opt_weight, delay_opt_weight, iteration, starting_transistor_sizes, spice_interface, 1, 0)
            else:
                sizing_results_dict[name] = sizing_results_list[len(
                    sizing_results_list)-1][name]
                sizing_results_detailed_dict[name] = sizing_results_detailed_list[len(
        ...
        sizing_results_detailed_list.append(
            sizing_results_detailed_dict.copy())
        ...
        print_results(opt_type, sizing_results_list,
                      area_results_list, delay_results_list)

        # Export these same results to a file
        sizing_results_filename = "sizing_results/sizing_results_o" + \
            str(iteration) + ".txt"
        export_sizing_results(
            sizing_results_filename, sizing_results_list, area_results_list, delay_results_list)

        # Check if transistor sizing is done
        is_done, final_result_index = check_if_done(
            sizing_results_list, area_results_list, delay_results_list, area_opt_weight, delay_opt_weight)
        ...
        final_report_file = open(
            "sizing_results/sizes_iteration_" + str(iteration) + ".txt", 'w')
...
def print_final_transistor_size(fpga_inst, report_file):
    ...
    for trans in fpga_inst.transistor_sizes:
        report_file.write(trans + "  =  " +
                          str(fpga_inst.transistor_sizes[trans]) + "\n")
