...
def print_vpr_file_memory(vpr_file, fpga_inst):
    ...
    # get areas
    grid_logic_tile_area = fpga_inst.area_dict["logic_cluster"] / \
        fpga_inst.specs.min_width_tran_area
    if grid_logic_tile_area < 1.00:
        grid_logic_tile_area = 1.00
    ipin_mux_trans_size = fpga_inst.area_dict["ipin_mux_trans_size"] / \
        fpga_inst.specs.min_width_tran_area
    if ipin_mux_trans_size < 1.00:
        ipin_mux_trans_size = 1.00
    mux_trans_size = fpga_inst.area_dict["switch_mux_trans_size"] / \
        fpga_inst.specs.min_width_tran_area
    if mux_trans_size < 1.00:
        mux_trans_size = 1.00
    buf_size = fpga_inst.area_dict["switch_buf_size"] / \
        fpga_inst.specs.min_width_tran_area
    if buf_size < 1.00:
        buf_size = 1.00
    ...
    memory_height = int(
        round(fpga_inst.area_dict["ram"]/fpga_inst.area_dict["tile"], 0))
    memory_max_width = 2 ** fpga_inst.RAM.conf_decoder_bits
    memory_address_bits = fpga_inst.specs.row_decoder_bits + \
        fpga_inst.specs.col_decoder_bits + fpga_inst.specs.conf_decoder_bits
    memory_max_depth = 2 ** (fpga_inst.specs.row_decoder_bits +
                             fpga_inst.specs.col_decoder_bits + fpga_inst.specs.conf_decoder_bits)
    ...
    cb_buf_size = fpga_inst.area_dict["cb_buf_size"] / \
        fpga_inst.specs.min_width_tran_area
    if cb_buf_size < 1.00:
        cb_buf_size = 1.00
    ...
    vpr_file.write("    <switch type=\"mux\" name=\"0\" R=\"0.000000\" Cin=\"0.000000e+00\" Cout=\"0.000000e+00\" Tdel=\"" +
                   str(Tdel) + "\" mux_trans_size=\"" + str(mux_trans_size) + "\" buf_size=\"" + str(buf_size) + "\"/>\n")
    vpr_file.write("    <switch type=\"mux\" name=\"ipin_cblock\" R=\"0.000000\" Cin=\"0.000000e+00\" Cout=\"0.000000e+00\" Tdel=\"" +
                   str(T_ipin_cblock) + "\" mux_trans_size=\"" + str(ipin_mux_trans_size) + "\" buf_size=\"" + str(cb_buf_size) + "\"/>\n")
    vpr_file.write("  </switchlist>\n")

    vpr_file.write("  <segmentlist>\n")
    vpr_file.write("    <segment freq=\"1.000000\" length=\"" + str(L) +
                   "\" type=\"unidir\" Rmetal=\"0.000000\" Cmetal=\"0.000000e+00\">\n")
    ...
    vpr_file.write("  <pb_type name=\"clb\" area=\"" +
                   str(grid_logic_tile_area)+"\">\n")
    vpr_file.write("    <input name=\"I1\" num_pins=\"" +
                   str(N) + "\" equivalent=\"true\"/>\n")
    vpr_file.write("    <input name=\"I2\" num_pins=\"" +
                   str(N) + "\" equivalent=\"true\"/>\n")
    vpr_file.write("    <input name=\"I3\" num_pins=\"" +
                   str(N) + "\" equivalent=\"true\"/>\n")
    vpr_file.write("    <input name=\"I4\" num_pins=\"" +
                   str(N) + "\" equivalent=\"true\"/>\n")
    vpr_file.write(
    ...
    vpr_file.write("  <input name=\"in\" num_pins=\"" +
                   str(K) + "\" port_class=\"lut_in\"/>\n")
    vpr_file.write(
    ...
    for input_name in lut_input_names:
        if Rfb.count(input_name) != 1:
            vpr_file.write("    <direct name=\"direct" + str(input_num) + "\" input=\"ble6.in_" +
                           input_name.upper() + "\" output=\"lut6.in[" + str(input_num) + ":" + str(input_num) + "]\">\n")
            vpr_file.write("      <delay_constant max=\"" + str(lut_delays[input_name]) + "\" in_port=\"ble6.in_" + input_name.upper(
            ) + "\" out_port=\"lut6.in[" + str(input_num) + ":" + str(input_num) + "]\" />\n")
            vpr_file.write("    </direct>\n")
    ...
    vpr_file.write("      <direct name=\"direct" + str(direct_num) +
                   "\" input=\"ble6.clk\" output=\"ff.clk\"/>\n")

    mux_num = 1
    for name in feedback_mux:
        vpr_file.write("      <!-- Register feedback mux -->   \n")
        vpr_file.write("      <mux name=\"mux" + str(mux_num) + "\" input=\"ble6.in_" + name.upper() +
                       " ff.Q\" output=\"lut6.in[" + str(feedback_mux[name]) + ":" + str(feedback_mux[name]) + "]\">\n")
        vpr_file.write("        <delay_constant max=\"" + str(lut_delays[name]) + "\" in_port=\"ble6.in_" + name.upper(
        ) + "\" out_port=\"lut6.in[" + str(feedback_mux[name]) + ":" + str(feedback_mux[name]) + "]\" />\n")
        vpr_file.write("        <delay_constant max=\"" + str(lut_delays[name]) + "\" in_port=\"ff.Q\" out_port=\"lut6.in[" + str(
            feedback_mux[name]) + ":" + str(feedback_mux[name]) + "]\" />  \n")
        vpr_file.write("      </mux>\n")
        mux_num = mux_num + 1
        vpr_file.write("      <!-- FF input selection mux -->\n")
        vpr_file.write("      <mux name=\"" + str(mux_num) +
                       "\" input=\"lut6.out ble6.in_" + name.upper() + "\" output=\"ff.D\">\n")
        vpr_file.write(
            "        <delay_constant max=\"1.74588e-11\" in_port=\"lut6.out\" out_port=\"ff.D\" />\n")
        vpr_file.write("        <delay_constant max=\"1.74588e-11\" in_port=\"ble6.in_" +
                       name.upper() + "\" out_port=\"ff.D\" />\n")
        vpr_file.write("      </mux>\n")
        mux_num = mux_num + 1

    vpr_file.write("      <!-- BLE output (local) -->\n")
    vpr_file.write("      <mux name=\"mux" + str(mux_num) +
                   "\" input=\"ff.Q lut6.out\" output=\"ble6.out_local\">\n")
    vpr_file.write("        <delay_constant max=\"" + str(local_feedback) +
                   "\" in_port=\"ff.Q\" out_port=\"ble6.out_local\" />\n")
    vpr_file.write("        <delay_constant max=\"" + str(local_feedback) +
                   "\" in_port=\"lut6.out\" out_port=\"ble6.out_local\" />\n")
    vpr_file.write("      </mux>\n")
    mux_num = mux_num + 1

    vpr_file.write("      <!-- BLE output (routing 1) --> \n")
    vpr_file.write("      <mux name=\"mux" + str(mux_num) +
                   "\" input=\"ff.Q lut6.out\" output=\"ble6.out_routing[0:0]\">\n")
    vpr_file.write("        <delay_constant max=\"" + str(logic_block_output) +
                   "\" in_port=\"ff.Q\" out_port=\"ble6.out_routing[0:0]\" />\n")
    vpr_file.write("        <delay_constant max=\"" + str(logic_block_output) +
                   "\" in_port=\"lut6.out\" out_port=\"ble6.out_routing[0:0]\" />\n")
    vpr_file.write("      </mux>\n")
    mux_num = mux_num + 1

    vpr_file.write("      <!-- BLE output (routing 2) --> \n")
    vpr_file.write("      <mux name=\"mux" + str(mux_num) +
                   "\" input=\"ff.Q lut6.out\" output=\"ble6.out_routing[1:1]\">\n")
    vpr_file.write("        <delay_constant max=\"" + str(logic_block_output) +
                   "\" in_port=\"ff.Q\" out_port=\"ble6.out_routing[1:1]\" />\n")
    vpr_file.write("        <delay_constant max=\"" + str(logic_block_output) +
                   "\" in_port=\"lut6.out\" out_port=\"ble6.out_routing[1:1]\" />\n")
    vpr_file.write("      </mux>\n")
    vpr_file.write("      </interconnect>\n")
    vpr_file.write("    </pb_type>\n")
    ...
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I4\" out_port=\"ble6.in_A\" />\n")
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I3\" out_port=\"ble6.in_A\" />\n")
    ...
    vpr_file.write("    </interconnect>\n")
    vpr_file.write("    <fc default_in_type=\"frac\" default_in_val=\"" + str(Fcin) +
                   "\" default_out_type=\"frac\" default_out_val=\"" + str(Fcout) + "\"/>\n")
    ...
    vpr_file.write("  <pb_type name=\"memory\" height=\""+str(memory_height)+"\" area=\"" +
                   str(fpga_inst.area_dict["ram_core"]/fpga_inst.specs.min_width_tran_area)+"\">\n")

    vpr_file.write("      <input name=\"addr1\" num_pins=\"" +
                   str(memory_address_bits)+"\"/>\n")
    vpr_file.write("      <input name=\"addr2\" num_pins=\"" +
                   str(memory_address_bits)+"\"/>\n")

    vpr_file.write("      <input name=\"data\" num_pins=\"" +
                   str(memory_max_width)+"\"/>\n")

    vpr_file.write("      <input name=\"we1\" num_pins=\"1\"/>\n")
    vpr_file.write("      <input name=\"we2\" num_pins=\"1\"/>\n")

    vpr_file.write("      <output name=\"out\" num_pins=\"" +
                   str(memory_max_width)+"\"/>\n")
    ...
    current_address_bits = fpga_inst.specs.row_decoder_bits + \
        fpga_inst.specs.col_decoder_bits - 1

    while current_width >= 1:
        vpr_file.write("      <mode name=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_sp\">\n")
        vpr_file.write("        <pb_type name=\"mem_"+str(current_depth)+"x"+str(current_width) +
                       "_sp\" blif_model=\".subckt single_port_ram\" class=\"memory\" num_pb=\"1\">\n")
        vpr_file.write("          <input name=\"addr\" num_pins=\"" +
                       str(current_address_bits)+"\" port_class=\"address\"/>\n")
        vpr_file.write("          <input name=\"data\" num_pins=\"" +
                       str(current_width)+"\" port_class=\"data_in\"/>\n")
        vpr_file.write(
            "          <input name=\"we\" num_pins=\"1\" port_class=\"write_en\"/>\n")
        vpr_file.write("          <output name=\"out\" num_pins=\"" +
                       str(current_width)+"\" port_class=\"data_out\"/>\n")
        vpr_file.write(
            "          <clock name=\"clk\" num_pins=\"1\" port_class=\"clock\"/>\n")
        vpr_file.write("          <T_setup value=\""+str(memory_setup_time)+"\" port=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_sp.addr\" clock=\"clk\"/>\n")
        vpr_file.write("          <T_setup value=\""+str(memory_setup_time)+"\" port=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_sp.data\" clock=\"clk\"/>\n")
        vpr_file.write("          <T_setup value=\""+str(memory_setup_time)+"\" port=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_sp.we\" clock=\"clk\"/>\n")
        vpr_file.write("          <T_clock_to_Q max=\""+str(memory_clktoq)+"\" port=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_sp.out\" clock=\"clk\"/>\n")
        vpr_file.write("        </pb_type>\n")
        vpr_file.write("        <interconnect>\n")
        vpr_file.write("          <direct name=\"address1\" input=\"memory.addr1["+str(
            current_address_bits - 1)+":0]\" output=\"mem_"+str(current_depth)+"x"+str(current_width)+"_sp.addr\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("          <direct name=\"data1\" input=\"memory.data["+str(
            current_width - 1)+":0]\" output=\"mem_"+str(current_depth)+"x"+str(current_width)+"_sp.data\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("          <direct name=\"writeen1\" input=\"memory.we1\" output=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_sp.we\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("          <direct name=\"dataout1\" input=\"mem_"+str(current_depth)+"x" +
                       str(current_width)+"_sp.out\" output=\"memory.out["+str(current_width - 1)+":0]\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("          <direct name=\"clk\" input=\"memory.clk\" output=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_sp.clk\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("        </interconnect>\n")
        vpr_file.write("      </mode>\n")
    ...
    current_depth = memory_max_depth / current_width
    current_address_bits = fpga_inst.specs.row_decoder_bits + \
        fpga_inst.specs.col_decoder_bits

    while current_width >= 1:

        vpr_file.write("      <mode name=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_dp\">\n")
        vpr_file.write("        <pb_type name=\"mem_"+str(current_depth)+"x"+str(current_width) +
                       "_dp\" blif_model=\".subckt dual_port_ram\" class=\"memory\" num_pb=\"1\">\n")
        vpr_file.write("          <input name=\"addr1\" num_pins=\"" +
                       str(current_address_bits)+"\" port_class=\"address1\"/>\n")
        vpr_file.write("          <input name=\"addr2\" num_pins=\"" +
                       str(current_address_bits)+"\" port_class=\"address2\"/>\n")
        vpr_file.write("          <input name=\"data1\" num_pins=\"" +
                       str(current_width)+"\" port_class=\"data_in1\"/>\n")
        vpr_file.write("          <input name=\"data2\" num_pins=\"" +
                       str(current_width)+"\" port_class=\"data_in2\"/>\n")
        vpr_file.write(
            "          <input name=\"we1\" num_pins=\"1\" port_class=\"write_en1\"/>\n")
        vpr_file.write(
            "          <input name=\"we2\" num_pins=\"1\" port_class=\"write_en2\"/>\n")
        vpr_file.write("          <output name=\"out1\" num_pins=\"" +
                       str(current_width)+"\" port_class=\"data_out1\"/>\n")
        vpr_file.write("          <output name=\"out2\" num_pins=\"" +
                       str(current_width)+"\" port_class=\"data_out2\"/>\n")
        vpr_file.write(
            "          <clock name=\"clk\" num_pins=\"1\" port_class=\"clock\"/>\n")
        vpr_file.write("          <T_setup value=\""+str(memory_setup_time)+"\" port=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_dp.addr1\" clock=\"clk\"/>\n")
        vpr_file.write("          <T_setup value=\""+str(memory_setup_time)+"\" port=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_dp.data1\" clock=\"clk\"/>\n")
        vpr_file.write("          <T_setup value=\""+str(memory_setup_time)+"\" port=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_dp.we1\" clock=\"clk\"/>\n")
        vpr_file.write("          <T_setup value=\""+str(memory_setup_time)+"\" port=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_dp.addr2\" clock=\"clk\"/>\n")
        vpr_file.write("          <T_setup value=\""+str(memory_setup_time)+"\" port=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_dp.data2\" clock=\"clk\"/>\n")
        vpr_file.write("          <T_setup value=\""+str(memory_setup_time)+"\" port=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_dp.we2\" clock=\"clk\"/>\n")
        vpr_file.write("          <T_clock_to_Q max=\""+str(memory_clktoq)+"\" port=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_dp.out1\" clock=\"clk\"/>\n")
        vpr_file.write("          <T_clock_to_Q max=\""+str(memory_clktoq)+"\" port=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_dp.out2\" clock=\"clk\"/>\n")
        vpr_file.write("        </pb_type>\n")
        vpr_file.write("        <interconnect>\n")
        vpr_file.write("          <direct name=\"address1\" input=\"memory.addr1["+str(
            current_address_bits - 1)+":0]\" output=\"mem_"+str(current_depth)+"x"+str(current_width)+"_dp.addr1\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("          <direct name=\"address2\" input=\"memory.addr2["+str(
            current_address_bits - 1)+":0]\" output=\"mem_"+str(current_depth)+"x"+str(current_width)+"_dp.addr2\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("          <direct name=\"data1\" input=\"memory.data["+str(
            current_width - 1)+":0]\" output=\"mem_"+str(current_depth)+"x"+str(current_width)+"_dp.data1\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("          <direct name=\"data2\" input=\"memory.data["+str(2*current_width - 1)+":"+str(
            current_width)+"]\" output=\"mem_"+str(current_depth)+"x"+str(current_width)+"_dp.data2\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("          <direct name=\"writeen1\" input=\"memory.we1\" output=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_dp.we1\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("          <direct name=\"writeen2\" input=\"memory.we2\" output=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_dp.we2\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("          <direct name=\"dataout1\" input=\"mem_"+str(current_depth)+"x" +
                       str(current_width)+"_dp.out1\" output=\"memory.out["+str(current_width - 1)+":0]\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("          <direct name=\"dataout2\" input=\"mem_"+str(current_depth)+"x"+str(current_width) +
                       "_dp.out2\" output=\"memory.out["+str(2*current_width - 1)+":"+str(current_width)+"]\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("          <direct name=\"clk\" input=\"memory.clk\" output=\"mem_" +
                       str(current_depth)+"x"+str(current_width)+"_dp.clk\">\n")
        vpr_file.write("          </direct>\n")
        vpr_file.write("        </interconnect>\n")
        vpr_file.write("      </mode>\n")
...
def print_vpr_file_flut_hard(vpr_file, fpga_inst):
    ...
    if independent_inputs > 0:
        vpr_file.write(
            "  <input name=\"in_E2\" num_pins=\"1\" equivalent=\"false\"/>\n")
    if independent_inputs > 1:
        vpr_file.write(
            "  <input name=\"in_D2\" num_pins=\"1\" equivalent=\"false\"/>\n")
    if independent_inputs > 2:
        vpr_file.write(
            "  <input name=\"in_C2\" num_pins=\"1\" equivalent=\"false\"/>\n")
    if independent_inputs > 3:
        vpr_file.write(
            "  <input name=\"in_B2\" num_pins=\"1\" equivalent=\"false\"/>\n")
    if independent_inputs > 4:
        vpr_file.write(
            "  <input name=\"in_A2\" num_pins=\"1\" equivalent=\"false\"/>\n")

    vpr_file.write(
        "  <output name=\"out_local\" num_pins=\"2\" equivalent=\"false\"/>\n")
    vpr_file.write(
        "  <output name=\"out_routing\" num_pins=\"2\" equivalent=\"true\"/>\n")
    vpr_file.write("  <clock name=\"clk\" num_pins=\"1\"/> \n")
    ...
    vpr_file.write(
        "  <output name=\"out_local\" num_pins=\"1\" equivalent=\"false\"/>\n")
    vpr_file.write(
        "  <output name=\"out_routing\" num_pins=\"1\" equivalent=\"true\"/>\n")
    vpr_file.write("  <clock name=\"clk\" num_pins=\"1\"/> \n")
    vpr_file.write(
    ...
    for input_name in lut_input_names:
        if Rfb.count(input_name) != 1 and input_num != 5:
            vpr_file.write("    <direct name=\"direct" + str(input_num) + "\" input=\"ble5.in_" +
                           input_name.upper() + "\" output=\"lut5.in[" + str(input_num) + ":" + str(input_num) + "]\">\n")
            vpr_file.write("      <delay_constant max=\"" + str(lut_delays[input_name]) + "\" in_port=\"ble5.in_" + input_name.upper(
            ) + "\" out_port=\"lut5.in[" + str(input_num) + ":" + str(input_num) + "]\" />\n")
            vpr_file.write("    </direct>\n")
    ...
    vpr_file.write("      <direct name=\"direct" + str(direct_num) +
                   "\" input=\"ble5.clk\" output=\"ff.clk\"/>\n")

    mux_num = 1
    for name in feedback_mux:
        vpr_file.write("      <!-- Register feedback mux -->   \n")
        vpr_file.write("      <mux name=\"mux" + str(mux_num) + "\" input=\"ble5.in_" + name.upper() +
                       " ff.Q\" output=\"lut5.in[" + str(feedback_mux[name]) + ":" + str(feedback_mux[name]) + "]\">\n")
        vpr_file.write("        <delay_constant max=\"" + str(lut_delays[name]) + "\" in_port=\"ble5.in_" + name.upper(
        ) + "\" out_port=\"lut5.in[" + str(feedback_mux[name]) + ":" + str(feedback_mux[name]) + "]\" />\n")
        vpr_file.write("        <delay_constant max=\"" + str(lut_delays[name]) + "\" in_port=\"ff.Q\" out_port=\"lut5.in[" + str(
            feedback_mux[name]) + ":" + str(feedback_mux[name]) + "]\" />  \n")
        vpr_file.write("      </mux>\n")
        mux_num = mux_num + 1
        vpr_file.write("      <!-- FF input selection mux -->\n")
        vpr_file.write("      <mux name=\"" + str(mux_num) +
                       "\" input=\"lut5.out ble5.in_" + name.upper() + "\" output=\"ff.D\">\n")
        vpr_file.write(
            "        <delay_constant max=\"1.74588e-11\" in_port=\"lut5.out\" out_port=\"ff.D\" />\n")
        vpr_file.write("        <delay_constant max=\"1.74588e-11\" in_port=\"ble5.in_" +
                       name.upper() + "\" out_port=\"ff.D\" />\n")
        vpr_file.write("      </mux>\n")
        mux_num = mux_num + 1
    ...
    vpr_file.write("      <!-- BLE output (local) -->\n")
    vpr_file.write("      <mux name=\"mux" + str(mux_num) +
                   "\" input=\"ff.Q lut5.out\" output=\"ble5.out_local\">\n")
    vpr_file.write("        <delay_constant max=\"" + str(local_feedback) +
                   "\" in_port=\"ff.Q\" out_port=\"ble5.out_local\" />\n")
    vpr_file.write("        <delay_constant max=\"" + str(local_feedback) +
                   "\" in_port=\"lut5.out\" out_port=\"ble5.out_local\" />\n")
    vpr_file.write("      </mux>\n")
    mux_num = mux_num + 1

    vpr_file.write("      <!-- BLE output (routing 1) --> \n")
    vpr_file.write("      <mux name=\"mux" + str(mux_num) +
                   "\" input=\"ff.Q lut5.out\" output=\"ble5.out_routing[0:0]\">\n")
    vpr_file.write("        <delay_constant max=\"" + str(logic_block_output) +
                   "\" in_port=\"ff.Q\" out_port=\"ble5.out_routing[0:0]\" />\n")
    vpr_file.write("        <delay_constant max=\"" + str(logic_block_output) +
                   "\" in_port=\"lut5.out\" out_port=\"ble5.out_routing[0:0]\" />\n")
    vpr_file.write("      </mux>\n")
    vpr_file.write(" </interconnect>\n")
    vpr_file.write("    </pb_type>\n")
    ...
    if independent_inputs > 4:
        vpr_file.write(
            " <direct name=\"direct6\" input=\"lut5inter.in_A2\" output=\"ble5[1:1].in_A\"/>\n")
    else:
        vpr_file.write(
    ...
    if independent_inputs > 3:
        vpr_file.write(
            " <direct name=\"direct7\" input=\"lut5inter.in_B2\" output=\"ble5[1:1].in_B\"/>\n")
    else:
        vpr_file.write(
    ...
    if independent_inputs > 2:
        vpr_file.write(
            " <direct name=\"direct8\" input=\"lut5inter.in_C2\" output=\"ble5[1:1].in_C\"/>\n")
    else:
        vpr_file.write(
    ...
    if independent_inputs > 1:
        vpr_file.write(
            " <direct name=\"direct9\" input=\"lut5inter.in_D2\" output=\"ble5[1:1].in_D\"/>\n")
    else:
        vpr_file.write(
    ...
    if independent_inputs > 0:
        vpr_file.write(
            " <direct name=\"direct10\" input=\"lut5inter.in_E2\" output=\"ble5[1:1].in_E\"/>\n")
    else:
        vpr_file.write(
    ...
    if independent_inputs > 0:
        vpr_file.write(
            " <direct name=\"direct10\" input=\"fle.in_E2\" output=\"lut5inter.in_E2\"/>\n")
    if independent_inputs > 1:
        vpr_file.write(
            " <direct name=\"direct11\" input=\"fle.in_D2\" output=\"lut5inter.in_D2\"/>\n")
    if independent_inputs > 2:
        vpr_file.write(
            " <direct name=\"direct12\" input=\"fle.in_C2\" output=\"lut5inter.in_C2\"/>\n")
    if independent_inputs > 3:
        vpr_file.write(
            " <direct name=\"direct13\" input=\"fle.in_B2\" output=\"lut5inter.in_B2\"/>\n")
    if independent_inputs > 4:
        vpr_file.write(
            " <direct name=\"direct14\" input=\"fle.in_A2\" output=\"lut5inter.in_A2\"/>\n")
    vpr_file.write(" </interconnect>\n")
    ...
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I4\" out_port=\"fle.in_A\" />\n")
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I3\" out_port=\"fle.in_A\" />\n")
    vpr_file.write("      </complete>\n")
    if independent_inputs > 4:
        vpr_file.write(
            "    <complete name=\"lutA2\" input=\"clb.I4 clb.I3 fle[1:0].out_local fle[3:2].out_local fle[8:8].out_local\" output=\"fle[9:0].in_A2\">\n")
        vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                       "\" in_port=\"clb.I4\" out_port=\"fle.in_A2\" />\n")
        vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                       "\" in_port=\"clb.I3\" out_port=\"fle.in_A2\" />\n")
        vpr_file.write("      </complete>\n")
    vpr_file.write(
        "    <complete name=\"lutB\" input=\"clb.I3 clb.I2 fle[3:2].out_local fle[5:4].out_local fle[9:9].out_local\" output=\"fle[9:0].in_B\">\n")
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I3\" out_port=\"fle.in_B\" />\n")
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I2\" out_port=\"fle.in_B\" />\n")
    vpr_file.write("      </complete>\n")
    if independent_inputs > 3:
        vpr_file.write(
            "    <complete name=\"lutB2\" input=\"clb.I3 clb.I2 fle[3:2].out_local fle[5:4].out_local fle[9:9].out_local\" output=\"fle[9:0].in_B2\">\n")
        vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                       "\" in_port=\"clb.I3\" out_port=\"fle.in_B2\" />\n")
        vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                       "\" in_port=\"clb.I2\" out_port=\"fle.in_B2\" />\n")
        vpr_file.write("      </complete>\n")
    vpr_file.write(
        "    <complete name=\"lutC\" input=\"clb.I2 clb.I1 fle[5:4].out_local fle[7:6].out_local fle[8:8].out_local\" output=\"fle[9:0].in_C\">\n")
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I2\" out_port=\"fle.in_C\" />\n")
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I1\" out_port=\"fle.in_C\" />\n")
    vpr_file.write("      </complete>\n")
    if independent_inputs > 2:
        vpr_file.write(
            "    <complete name=\"lutC2\" input=\"clb.I2 clb.I1 fle[5:4].out_local fle[7:6].out_local fle[8:8].out_local\" output=\"fle[9:0].in_C2\">\n")
        vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                       "\" in_port=\"clb.I2\" out_port=\"fle.in_C2\" />\n")
        vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                       "\" in_port=\"clb.I1\" out_port=\"fle.in_C2\" />\n")
        vpr_file.write("      </complete>\n")
    vpr_file.write(
        "    <complete name=\"lutD\" input=\"clb.I4 clb.I2 fle[1:0].out_local fle[5:4].out_local fle[9:9].out_local\" output=\"fle[9:0].in_D\">\n")
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I4\" out_port=\"fle.in_D\" />\n")
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I2\" out_port=\"fle.in_D\" />\n")
    vpr_file.write("      </complete>\n")
    if independent_inputs > 1:
        vpr_file.write(
            "    <complete name=\"lutD2\" input=\"clb.I4 clb.I2 fle[1:0].out_local fle[5:4].out_local fle[9:9].out_local\" output=\"fle[9:0].in_D2\">\n")
        vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                       "\" in_port=\"clb.I4\" out_port=\"fle.in_D2\" />\n")
        vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                       "\" in_port=\"clb.I2\" out_port=\"fle.in_D2\" />\n")
        vpr_file.write("      </complete>\n")
    vpr_file.write(
        "    <complete name=\"lutE\" input=\"clb.I3 clb.I1 fle[3:2].out_local fle[7:6].out_local fle[8:8].out_local\" output=\"fle[9:0].in_E\">\n")
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I3\" out_port=\"fle.in_E\" />\n")
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I1\" out_port=\"fle.in_E\" />\n")
    vpr_file.write("      </complete>\n")
    if independent_inputs > 0:
        vpr_file.write(
            "    <complete name=\"lutE2\" input=\"clb.I3 clb.I1 fle[3:2].out_local fle[7:6].out_local fle[8:8].out_local\" output=\"fle[9:0].in_E2\">\n")
        vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                       "\" in_port=\"clb.I3\" out_port=\"fle.in_E2\" />\n")
        vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                       "\" in_port=\"clb.I1\" out_port=\"fle.in_E2\" />\n")
        vpr_file.write("      </complete>\n")
    vpr_file.write(
        "    <complete name=\"lutF\" input=\"clb.I4 clb.I1 fle[1:0].out_local fle[7:6].out_local fle[9:9].out_local\" output=\"fle[9:0].in_F\">\n")
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I4\" out_port=\"fle.in_F\" />\n")
    vpr_file.write("      <delay_constant max=\"" + str(local_CLB_routing) +
                   "\" in_port=\"clb.I1\" out_port=\"fle.in_F\" />\n")
    vpr_file.write("      </complete>\n")
    vpr_file.write(
    ...
    vpr_file.write("          <input name=\"addr1\" num_pins=\"" +
                   str(N) + "\" port_class=\"address1\"/>\n")
    vpr_file.write("          <input name=\"addr2\" num_pins=\"" +
                   str(N) + "\" port_class=\"address2\"/>\n")
    vpr_file.write(
