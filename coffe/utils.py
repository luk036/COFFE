...
def print_area_and_delay(report_file, fpga_inst):
    ...
    # Print the header
    print_and_write(report_file, "  Subcircuit".ljust(32) + "Area (um^2)".ljust(MIDL_COL_WIDTH) + "Delay (ps)".ljust(MIDL_COL_WIDTH) +
                    "tfall (ps)".ljust(MIDL_COL_WIDTH) + "trise (ps)".ljust(MIDL_COL_WIDTH) + "Power at 250MHz (uW)".ljust(LAST_COL_WIDTH))
    ...
    lut_input_names = list(
        fpga_inst.logic_cluster.ble.lut.input_drivers.keys())
    ...
    if fpga_inst.specs.enable_carry_chain == 1:
        ...
        print_and_write(report_file, "  " + "total carry chain area".ljust(FIRS_COL_WIDTH) +
                        str(round(area_dict["total_carry_chain"]/1e6, 3)).ljust(MIDL_COL_WIDTH))
    ...
    print("  Power Breakdown: ".ljust(FIRS_COL_WIDTH) + "stage0".ljust(24) +
          str(round(fpga_inst.RAM.rowdecoder_stage0.power/1e-6, 4)).ljust(LAST_COL_WIDTH))

    if fpga_inst.RAM.valid_row_dec_size2 == 1:
        print("  Power Breakdown: ".ljust(FIRS_COL_WIDTH) + "stage1".ljust(24) +
              str(round(fpga_inst.RAM.rowdecoder_stage1_size2.power/1e-6, 4)).ljust(LAST_COL_WIDTH))
    if fpga_inst.RAM.valid_row_dec_size3 == 1:
        print("  Power Breakdown: ".ljust(FIRS_COL_WIDTH) + "stage1".ljust(24) +
              str(round(fpga_inst.RAM.rowdecoder_stage1_size3.power/1e-6, 4)).ljust(LAST_COL_WIDTH))
    print("  Power Breakdown: ".ljust(FIRS_COL_WIDTH) + "stage2".ljust(24) +
          str(round(fpga_inst.RAM.rowdecoder_stage3.power/1e-6, 4)).ljust(LAST_COL_WIDTH))
    ...
    if fpga_inst.RAM.cvalidobj1 != 0 and fpga_inst.RAM.cvalidobj2 != 0:
        configdelay += max(fpga_inst.RAM.configurabledecoder3ii.delay,
                           fpga_inst.RAM.configurabledecoder2ii.delay)
    ...
    print("  Power Breakdown: ".ljust(FIRS_COL_WIDTH) + "stage0".ljust(24) +
          str(round(fpga_inst.RAM.configurabledecoderi.power/1e-6, 4)).ljust(LAST_COL_WIDTH))
    if fpga_inst.RAM.cvalidobj2 != 0:
        print("  Power Breakdown: ".ljust(FIRS_COL_WIDTH) + "stage1".ljust(24) +
              str(round(fpga_inst.RAM.configurabledecoder2ii.power/1e-6, 4)).ljust(LAST_COL_WIDTH))
    if fpga_inst.RAM.cvalidobj1 != 0:
        print("  Power Breakdown: ".ljust(FIRS_COL_WIDTH) + "stage1".ljust(24) +
              str(round(fpga_inst.RAM.configurabledecoder3ii.power/1e-6, 4)).ljust(LAST_COL_WIDTH))
    print("  Power Breakdown: ".ljust(FIRS_COL_WIDTH) + "stage2".ljust(24) +
          str(round(fpga_inst.RAM.configurabledecoderiii.power/1e-6, 4)).ljust(LAST_COL_WIDTH))
...
def print_power(report_file, fpga_inst):
    ...
    print("  " + fpga_inst.sb_mux.name.ljust(22) +
          str(fpga_inst.sb_mux.power/1e-6))
    print("  " + fpga_inst.cb_mux.name.ljust(22) +
          str(fpga_inst.cb_mux.power/1e-6))
    print("  " + fpga_inst.logic_cluster.local_mux.name.ljust(22) +
          str(fpga_inst.logic_cluster.local_mux.power/1e-6))
    print("  " + fpga_inst.logic_cluster.ble.local_output.name.ljust(22) +
          str(fpga_inst.logic_cluster.ble.local_output.power/1e-6))
    print("  " + fpga_inst.logic_cluster.ble.general_output.name.ljust(22) +
          str(fpga_inst.logic_cluster.ble.general_output.power/1e-6))
    ...
    for input_name in lut_input_names:
        ...
        print("  " + ("lut_" + input_name).ljust(22) +
              str((path_power + driver_power + not_driver_power)/1e-6))
        print("  " + ("  lut_" + input_name +
              "_data_path").ljust(22) + str(path_power/1e-6))
        print("  " + ("  lut_" + input_name + "_driver").ljust(22) +
              str(driver_power/1e-6))
        print("  " + ("  lut_" + input_name + "_driver_not").ljust(22) +
              str(not_driver_power/1e-6))
...
def print_block_area(report_file, fpga_inst):
    ...
    if fpga_inst.specs.enable_bram_block == 1:
        ...
        ramconfdecode = (
            fpga_inst.area_dict["configurabledecoder"] * 2)/1000000
        ...
        if fpga_inst.RAM.memory_technology == "SRAM":
            ram_routing = ram - decod - ramlocalmux - ramcoldecode - ramconfdecode - ramoutputcbar - \
                prechargetotal - writedrivertotal - samptotal - \
                memcells - wordlinedrivera - levels
        else:
            ram_routing = ram - decod - ramlocalmux - ramcoldecode - ramconfdecode - ramoutputcbar - \
                memcells - wordlinedrivera - writedrivertotal - samptotal - cstotal - levels

    print_and_write(report_file, "  TILE AREA CONTRIBUTIONS")
    print_and_write(report_file, "  -----------------------")
    print_and_write(report_file, "  Block".ljust(
        20) + "Total Area (um^2)".ljust(20) + "Fraction of total tile area")
    print_and_write(report_file, "  Tile".ljust(
        20) + str(round(tile, 3)).ljust(20) + "100%")
    print_and_write(report_file, "  LUT".ljust(
        20) + str(round(lut, 3)).ljust(20) + str(round(lut/tile*100, 3)) + "%")
    print_and_write(report_file, "  FF".ljust(
        20) + str(round(ff, 3)).ljust(20) + str(round(ff/tile*100, 3)) + "%")
    print_and_write(report_file, "  BLE output".ljust(
        20) + str(round(ble_output, 3)).ljust(20) + str(round(ble_output/tile*100, 3)) + "%")
    print_and_write(report_file, "  Local mux".ljust(
        20) + str(round(local_mux, 3)).ljust(20) + str(round(local_mux/tile*100, 3)) + "%")
    print_and_write(report_file, "  Connection block".ljust(
        20) + str(round(cb, 3)).ljust(20) + str(round(cb/tile*100, 3)) + "%")
    print_and_write(report_file, "  Switch block".ljust(
        20) + str(round(sb, 3)).ljust(20) + str(round(sb/tile*100, 3)) + "%")
    print_and_write(report_file, "")
    if fpga_inst.specs.enable_bram_block == 1:
        print_and_write(report_file, "  RAM AREA CONTRIBUTIONS")
        print_and_write(report_file, "  -----------------------")
        print_and_write(report_file, "  Block".ljust(
            20) + "Total Area (um^2)".ljust(20) + "Fraction of RAM tile area")
        print_and_write(report_file, "  RAM".ljust(
            20) + str(round(ram, 3)).ljust(20) + str(round(ram/ram*100, 3)) + "%")
        print_and_write(report_file, "  RAM Local Mux".ljust(
            20) + str(round(ramlocalmux, 3)).ljust(20) + str(round(ramlocalmux/ram*100, 3)) + "%")
        print_and_write(report_file, "  Level Shifters".ljust(
            20) + str(round(levels, 3)).ljust(20) + str(round(levels/ram*100, 3)) + "%")
        print_and_write(report_file, "  Decoder".ljust(
            20) + str(round(decod, 3)).ljust(20) + str(round(decod/ram*100, 3)) + "%")
        print_and_write(report_file, "  WL driver".ljust(
            20) + str(round(wordlinedrivera, 3)).ljust(20) + str(round(wordlinedrivera/ram*100, 3)) + "%")
        print_and_write(report_file, "  Column Decoder".ljust(
            20) + str(round(ramcoldecode, 3)).ljust(20) + str(round(ramcoldecode/ram*100, 3)) + "%")
        print_and_write(report_file, "  Configurable Dec".ljust(
            20) + str(round(ramconfdecode, 3)).ljust(20) + str(round(ramconfdecode/ram*100, 3)) + "%")
        print_and_write(report_file, "  Output CrossBar".ljust(
            20) + str(round(ramoutputcbar, 3)).ljust(20) + str(round(ramoutputcbar/ram*100, 3)) + "%")
        if fpga_inst.RAM.memory_technology == "SRAM":
            print_and_write(report_file, "  Precharge Total".ljust(
                20) + str(round(prechargetotal, 3)).ljust(20) + str(round(prechargetotal/ram*100, 3)) + "%")
            print_and_write(report_file, "  Write Drivers".ljust(
                20) + str(round(writedrivertotal, 3)).ljust(20) + str(round(writedrivertotal/ram*100, 3)) + "%")
            print_and_write(report_file, "  Sense Amp Total ".ljust(
                20) + str(round(samptotal, 3)).ljust(20) + str(round(samptotal/ram*100, 3)) + "%")
        else:
            print_and_write(report_file, "  Column selectors".ljust(
                20) + str(round(cstotal, 3)).ljust(20) + str(round(cstotal/ram*100, 3)) + "%")
            print_and_write(report_file, "  Write Drivers".ljust(
                20) + str(round(writedrivertotal, 3)).ljust(20) + str(round(writedrivertotal/ram*100, 3)) + "%")
            print_and_write(report_file, "  Sense Amp Total ".ljust(
                20) + str(round(samptotal, 3)).ljust(20) + str(round(samptotal/ram*100, 3)) + "%")

        print_and_write(report_file, "  Memory Cells ".ljust(
            20) + str(round(memcells, 3)).ljust(20) + str(round(memcells/ram*100, 3)) + "%")
        print_and_write(report_file, "  RAM Routing".ljust(
            20) + str(round(ram_routing, 3)).ljust(20) + str(round(ram_routing/ram*100, 3)) + "%")
        print_and_write(report_file, "  RAM CB".ljust(
            20) + str(round(RAM_CB_TOTAL, 3)).ljust(20) + str(round(RAM_CB_TOTAL/ram*100, 3)) + "%")
        print_and_write(report_file, "  RAM SB".ljust(
            20) + str(round(RAM_SB_TOTAL, 3)).ljust(20) + str(round(RAM_SB_TOTAL/ram*100, 3)) + "%")
        print_and_write(report_file, "")
...
def print_hardblock_info(report_file, fpga_inst):
    ...
    for hardblock in fpga_inst.hardblocklist:
        ...
        print_and_write(report_file, "  Core area: " +
                        str(hardblock.area/1000000))
        print_and_write(report_file, "  Local mux area: " + str(
            hardblock.parameters['num_gen_inputs'] * fpga_inst.area_dict[hardblock.mux.name]/1000000))
        print_and_write(report_file, "  Local mux area with sram: " + str(
            hardblock.parameters['num_gen_inputs'] * fpga_inst.area_dict[hardblock.mux.name + "_sram"]/1000000))
        if hardblock.parameters['num_dedicated_outputs'] > 0:
            print_and_write(report_file, "  Dedicated output routing area: " + str(
                hardblock.parameters['num_dedicated_outputs'] * fpga_inst.area_dict[hardblock.name + "_ddriver"]/1000000))
        print_and_write(report_file, "  Total area: " +
                        str(fpga_inst.area_dict[hardblock.name + "_sram"]/1000000))
        print_and_write(report_file, "")
...
def print_vpr_delays(report_file, fpga_inst):
    ...
    print_and_write(report_file, "  Tdel (routing switch)".ljust(
        50) + str(fpga_inst.sb_mux.delay))
    print_and_write(report_file, "  T_ipin_cblock (connection block mux)".ljust(
        50) + str(fpga_inst.cb_mux.delay))
    print_and_write(report_file, "  CLB input -> BLE input (local CLB routing)".ljust(
        50) + str(fpga_inst.logic_cluster.local_mux.delay))
    print_and_write(report_file, "  LUT output -> BLE input (local feedback)".ljust(
        50) + str(fpga_inst.logic_cluster.ble.local_output.delay))
    print_and_write(report_file, "  LUT output -> CLB output (logic block output)".ljust(
        50) + str(fpga_inst.logic_cluster.ble.general_output.delay))
    ...
    for input_name in lut_input_names:
        ...
        print_and_write(
            report_file, ("  lut_" + input_name).ljust(50) + str(driver_delay+path_delay))

    if fpga_inst.specs.enable_bram_block == 1:
        print_and_write(report_file, "  RAM block frequency".ljust(
            50) + str(fpga_inst.RAM.frequency))
...
def print_vpr_areas(report_file, fpga_inst):
    ...
    print_and_write(report_file, "  ----------")
    print_and_write(report_file, "  grid_logic_tile_area".ljust(
        50) + str(fpga_inst.area_dict["logic_cluster"]/fpga_inst.specs.min_width_tran_area))
    print_and_write(report_file, "  ipin_mux_trans_size (connection block mux)".ljust(
        50) + str(fpga_inst.area_dict["ipin_mux_trans_size"]/fpga_inst.specs.min_width_tran_area))
    print_and_write(report_file, "  mux_trans_size (routing switch)".ljust(
        50) + str(fpga_inst.area_dict["switch_mux_trans_size"]/fpga_inst.specs.min_width_tran_area))
    print_and_write(report_file, "  buf_size (routing switch)".ljust(
        50) + str(fpga_inst.area_dict["switch_buf_size"]/fpga_inst.specs.min_width_tran_area))
    print_and_write(report_file, "")
...
def load_arch_params(filename):
    ...
    for line in params_file:
        ...
        if words[0] not in list(arch_params.keys()):
            print("ERROR: Found invalid architecture parameter (" +
                  words[0] + ") in " + filename)
            sys.exit()
    ...
    for param, value in arch_params.items():
        if value == -1 or value == "":
            print("ERROR: Did not find architecture parameter " +
                  param + " in " + filename)
            sys.exit()
...
def load_hard_params(filename):
    ...
    for line in hard_file:
        ...
        if words[0] not in list(hard_params.keys()):
            print("ERROR: Found invalid hard block parameter (" +
                  words[0] + ") in " + filename)
            sys.exit()
        ...
        if param == 'name':
            ...
            hard_params['tilehi_tielo_cells_between_power_gnd'] = (
                value == "True")
...
def check_arch_params(arch_params, filename):
    ...
    if arch_params['Rfb'] == 'z':
        ...
        print_error(arch_params['Rfb'], "Rfb", filename,
                    "(you specified more Rfb LUT inputs than there are LUT inputs)")
    else:
        ...
        for character in arch_params['Rfb']:
            ...
            if (character < 'a' or character > chr(arch_params['K']+96)):
                print_error(arch_params['Rfb'], "Rfb", filename,
                            " (" + character + " is not a valid LUT input)")
    ...
    if arch_params['FAs_per_flut'] > 2:
        print_error(str(arch_params['FAs_per_flut']), "FAs_per_flut",
                    filename, "(number of FA per ble should be 2 or less)")
    ...
    if arch_params['transistor_type'] != 'bulk' and arch_params['transistor_type'] != 'finfet':
        print_error(arch_params['transistor_type'],
                    "transistor_type", filename)
    ...
    if arch_params['rest_length_factor'] <= 0:
        print_error(str(arch_params['rest_length_factor']),
                    "rest_length_factor", filename)
    if arch_params['min_tran_width'] <= 0:
        print_error(str(arch_params['min_tran_width']),
                    "min_tran_width", filename)
    if arch_params['min_width_tran_area'] <= 0:
        print_error(str(arch_params['min_width_tran_area']),
                    "min_width_tran_area", filename)
    if arch_params['sram_cell_area'] <= 0:
        print_error(str(arch_params['sram_cell_area']),
                    "sram_cell_area", filename)
    if arch_params['trans_diffusion_length'] <= 0:
        print_error(
            str(arch_params['trans_diffusion_length']), "trans_diffusion_length", filename)
...
def print_error(value, argument, filename, msg=""):
    print("ERROR: Invalid value (" + value + ") for " +
          argument + " in " + filename + " " + msg)
    sys.exit()


def print_error_not_compatable(value1, value2):
    print("ERROR: " + value1 + " and " + value2 +
          " simulations are not compatible.\n")
    sys.exit()
...
def print_run_options(args, report_file_path):
    ...
    print_and_write(
        report_file, "  Number of top combos to re-ERF: " + str(args.re_erf))
    print_and_write(report_file, "  Area optimization weight: " +
                    str(args.area_opt_weight))
    print_and_write(
        report_file, "  Delay optimization weight: " + str(args.delay_opt_weight))
    print_and_write(
        report_file, "  Maximum number of sizing iterations: " + str(args.max_iterations))
    print_and_write(report_file, "")
    print_and_write(report_file, "")
...
def print_architecture_params(arch_params_dict, report_file_path):
    ...
    print_and_write(
        report_file, "  Number of BLEs per cluster (N): " + str(arch_params_dict['N']))
    print_and_write(report_file, "  LUT size (K): " +
                    str(arch_params_dict['K']))

    if arch_params_dict['use_fluts'] == True:
        print_and_write(report_file, "  LUT fracturability level: 1")
        print_and_write(report_file, "  Number of adder bits per ALM: " +
                        str(arch_params_dict['FAs_per_flut']))

    print_and_write(report_file, "  Channel width (W): " +
                    str(arch_params_dict['W']))
    print_and_write(report_file, "  Wire segment length (L): " +
                    str(arch_params_dict['L']))
    print_and_write(
        report_file, "  Number of cluster inputs (I): " + str(arch_params_dict['I']))
    print_and_write(
        report_file, "  Number of BLE outputs to general routing (Or): " + str(arch_params_dict['Or']))
    print_and_write(
        report_file, "  Number of BLE outputs to local routing (Ofb): " + str(arch_params_dict['Ofb']))
    print_and_write(report_file, "  Total number of cluster outputs (N*Or): " +
                    str(arch_params_dict['N']*arch_params_dict['Or']))
    print_and_write(
        report_file, "  Switch block flexibility (Fs): " + str(arch_params_dict['Fs']))
    print_and_write(
        report_file, "  Cluster input flexibility (Fcin): " + str(arch_params_dict['Fcin']))
    print_and_write(report_file, "  Cluster output flexibility (Fcout): " +
                    str(arch_params_dict['Fcout']))
    print_and_write(report_file, "  Local MUX population (Fclocal): " +
                    str(arch_params_dict['Fclocal']))
    print_and_write(
        report_file, "  LUT input for register selection MUX (Rsel): " + str(arch_params_dict['Rsel']))
    print_and_write(
        report_file, "  LUT input(s) for register feedback MUX(es) (Rfb): " + str(arch_params_dict['Rfb']))
    print_and_write(report_file, "")
    ...
    print_and_write(report_file, "  transistor_type = " +
                    arch_params_dict['transistor_type'])
    print_and_write(report_file, "  switch_type = " +
                    arch_params_dict['switch_type'])
    print_and_write(report_file, "  vdd = " + str(arch_params_dict['vdd']))
    print_and_write(report_file, "  vsram = " + str(arch_params_dict['vsram']))
    print_and_write(report_file, "  vsram_n = " +
                    str(arch_params_dict['vsram_n']))
    print_and_write(report_file, "  gate_length = " +
                    str(arch_params_dict['gate_length']))
    print_and_write(report_file, "  min_tran_width = " +
                    str(arch_params_dict['min_tran_width']))
    print_and_write(report_file, "  min_width_tran_area = " +
                    str(arch_params_dict['min_width_tran_area']))
    print_and_write(report_file, "  sram_cell_area = " +
                    str(arch_params_dict['sram_cell_area']))
    print_and_write(report_file, "  model_path = " +
                    str(arch_params_dict['model_path']))
    print_and_write(report_file, "  model_library = " +
                    str(arch_params_dict['model_library']))
...
def check_for_time():
    ...
    if (now.hour == 2 or now.hour == 3):
        print("-----------------------------------------------------------------")
        print("      Exited the check for time function  @ " +
              str(now.hour) + ":" + str(now.minute) + ":" + str(now.second))
        print("-----------------------------------------------------------------")
        print("")
...
def print_summary(arch_folder, fpga_inst, start_time):
    ...
    print_and_write(
        report_file, "|--------------------------------------------------------------------------------------------------|")
    print_and_write(
        report_file, "|    Area and Delay Report                                                                         |")
    print_and_write(
        report_file, "|--------------------------------------------------------------------------------------------------|")
    print_and_write(report_file, "")
    ...
    final_cost = fpga_inst.area_dict["tile"] * \
        fpga_inst.delay_dict["rep_crit_path"]

    print_and_write(report_file, "  SUMMARY")
    print_and_write(report_file, "  -------")
    print_and_write(report_file, "  Tile Area                            " +
                    str(round(fpga_inst.area_dict["tile"]/1e6, 2)) + " um^2")
    print_and_write(report_file, "  Representative Critical Path Delay   " +
                    str(round(fpga_inst.delay_dict["rep_crit_path"]*1e12, 2)) + " ps")
    ...
    total_minutes_elapsed = int(
        (total_time_elapsed - 3600*total_hours_elapsed)/60)
    total_seconds_elapsed = int(
        total_time_elapsed - 3600*total_hours_elapsed - 60*total_minutes_elapsed)

    print_and_write(report_file, "Number of HSPICE simulations performed: " +
                    str(fpga_inst.spice_interface.get_num_simulations_performed()))
    print_and_write(report_file, "Total time elapsed: " + str(total_hours_elapsed) + " hours " +
                    str(total_minutes_elapsed) + " minutes " + str(total_seconds_elapsed) + " seconds\n")
